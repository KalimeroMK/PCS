...
if (!G5_IS_MOBILE) {
    ...
    <table class="main">
        ...
        <tr>
            <td class="main_td td_sub" style="height:80px;" colspan="6"><a
                        href='javascript:document.submit_for.submit()'> <b> P&ID INFORMATION </b> </a></td>
        ...
        <tr>
            ...
            <td class="main_td" colspan="2">
                ...
                if ($pnid_state != 'Approved' && $member['mb_3'] > 0) { ?>
                    ...
                    <form name="numbering_form" action="<?php echo PCS_CORE_URL; ?>/pcs_mark_pnid_pdf.php" method="post"
        ...
        if ($pkg_array !== []) {
            ...
            $qty = count($pkg_array);
            if ($qty > 0) {
                echo '<td class="main_td" colspan=6 style="background-color: #FFBF00; height:50px;"><b>INCLUDED PACKAGE</td></tr>';
                ...
                foreach ($pkg_array as $pkg_no) {
                    ...
                    if ($sql_inc_dwg_arr['wr_id']) {
                        echo '<a href=' . G5_BBS_URL . '/board.php?bo_table=package&wr_id=' . $sql_inc_dwg_arr['wr_id'] . '> <b>' . $pkg_no . '</b></a>';
                    } else {
                        echo '<mark>' . $pkg_no . '</mark>';
                    }
                    echo '</a></td>';
                    if ($j % 6 == 0) {
                        echo '</tr><tr>';
