...
// -----------------------------------------------------------------------------
        ...
        if ($params[11] < $nd) {
            ...
            if ($params[11] - $nd > 1 && $cw[($nd - 1)] != 254) {
                if ($this->last_enc == ENC_EDF) {
                    ...
                    $cw[] = 124;
                    ++$nd;
                } elseif ($this->last_enc != ENC_ASCII && $this->last_enc != ENC_BASE256) {
                    // switch to ASCII encoding
                    $cw[] = 254;
                    ++$nd;
                }
    ...
    protected function getHighLevelEncoding($data)
    {
        ...
        while ($pos < $data_length) {
            ...
            switch ($enc) {
                case ENC_ASCII:
                    if ($data_length > 1 && $pos < $data_length - 1 && ($this->isCharMode(ord($data[$pos]), ENC_ASCII_NUM) && $this->isCharMode(ord($data[$pos + 1]), ENC_ASCII_NUM))) {
                ...
                case ENC_X12:
                    ...
                    do {
                        ...
                        if (isset($charset[$chr])) {
                            ...
                            } elseif ($enc == ENC_C40 && isset($this->chset['S3C'][$chr])) {
                                $temp_cw[] = 2; // shift 3
                                $shiftset = $this->chset['S3C'];
                            } elseif ($enc == ENC_TXT && isset($this->chset['S3T'][$chr])) {
                                $temp_cw[] = 2; // shift 3
                    ...
                    if ($numch[ENC_C40] == $numch[ENC_X12]) {
                        ...
                        while ($k < $data_length) {
                            ...
                            if ($this->isCharMode($tmpchr, ENC_X12)) {
                                return ENC_X12;
                            } elseif (!$this->isCharMode($tmpchr, ENC_X12) && !$this->isCharMode($tmpchr, ENC_C40)) {
                                break;
                            }
                            ++$k;
                        }
