    ...
    <table class="main">
        ...
        for ($i = 0; $i < $fld_qty; $i++) {
            if ($i == $fld_qty - 1) {
                ?>
                <tr>
                    <td class="main_td td_sub" style="width:25%"> <?php if ($field_arr[$i] == 'ag_ug') {
                            echo 'LEVEL';
                        } else {
                            echo strtoupper(str_replace('_', ' ', $field_arr[$i]));
                        } ?> </td>
                    <td class="main_td"><input type="text" name="<?php echo $field_arr[$i]; ?>"
                                               value="<?php echo $field_enum_value2[$field_arr[$i]]; ?>"></td>
                </tr>
                <?php
            } else {
                ?>
                <tr>
                    ...
                    <td class="main_td"><input type="text" name="<?php echo $field_arr[$i]; ?>"
                                               value="<?php echo $field_enum_value1[$field_arr[$i]]; ?>"></td>
                </tr>
                <?php
            }
        }
