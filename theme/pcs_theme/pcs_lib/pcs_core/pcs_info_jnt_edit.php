                ...
                for ($col = 0; $col < $field_cnt; $col++) {
                    ...
                    <td class="jnt_td">
                        ...
                        if ($col !== 0) {
                            ...
                            if ($col == 1 || $col == 2) {
                                echo 'onchange="fill_Jinfo(' . $row . ')" onclick=\'javascript:this.value="";\' ';
                            }
                            ...
                            if (($col == 2 && ($jnt_type == 'BT' || $jnt_type == 'PS' || $jnt_type == 'TH'))) {
                                echo 'disabled';
                            }
                            echo '>';
                            if ($col > 3) {
						...
						value="' . $sql_ref_dwg_arr[$field_arr[$col]] . '"> <option value="' . $main_dwg . '" >' . $main_dwg . '</option>';
                            for ($codwg = 0; $codwg < $cont_dwg_qty - 1; $codwg++) {
                                if ($sql_ref_dwg_arr['item_1_dwg'] == $cont_dwg[$codwg]) {
                                    echo '<option value="' . $cont_dwg[$codwg] . '" selected>' . $cont_dwg[$codwg] . '</option>';
                                } else {
                                    echo '<option value="' . $cont_dwg[$codwg] . '">' . $cont_dwg[$codwg] . '</option>';
                                }
                            }
                            echo '</select>';
                        }
...
<script language="javascript">
    ...
    function fill_Jinfo(row) {
        ...
        if (jntype == 'BT' || jntype == 'TH' || jntype == 'PS') {
            fill_txt(row, jntype, item1size1, '-', '-', '-', '-', '-');
        } else {
            var itemtext = item1type + item2type;

            if ((item1type == 'PIPE' && item2type == 'PIPE') || (item1type == 'NIPPLE' && item2type == 'NIPPLE')) {
                if (item1size1 > item2size1) {
                    fill_txt(row, 'BR', item2size1, '-', item1type, '-', item2type, '-');
                } else if (item1size1 == item2size1) {
                    fill_txt(row, item1jtyp1, item1size1, item1sche1, item1type, '-', item2type, '-');
                } else {
                    fill_txt(row, 'BR', item1size1, '-', item1type, '-', item2type, '-');
                }
            } else if (item1type == 'PIPE' || item1type == 'NIPPLE') {
                if (item2type.indexOf('OLET') > 0) {
                    if (item1size1 == item2size1) {
                        fill_txt(row, item2jtyp2, item1size1, item1sche1, item1type, '-', item2type, '-');
                    } else {
                        fill_txt(row, item2jtyp1, item2size1, item2sche1, item1type, '-', item2type, '-');
                    }
                } else if (item2type == 'FLANGE') {
                    fill_txt(row, item2jtyp1, item2size1, item1sche1, item1type, '-', item2type, '-');
                } else if (item1size1 == item2size1) {
                    fill_txt(row, item2jtyp1, item2size1, item2sche1, item1type, '-', item2type, '-');
                } else if (item1size1 == item2size2) {
                    fill_txt(row, item2jtyp2, item2size2, item2sche2, item1type, '-', item2type, '-');
                }
            } else if (item2type == 'PIPE' || item2type == 'NIPPLE') {
                if (item1type.indexOf('OLET') > 0) {
                    if (item1size1 == item2size1) {
                        fill_txt(row, item1jtyp2, item1size1, item1sche1, item1type, '-', item2type, '-');
                    } else {
                        fill_txt(row, item1jtyp1, item1size1, item1sche1, item1type, '-', item2type, '-');
                    }
                } else if (item1type == 'FLANGE') {
                    fill_txt(row, item1jtyp1, item2size1, item2sche1, item1type, '-', item2type, '-');
                } else if (item1size1 == item2size1) {
                    fill_txt(row, item1jtyp1, item1size1, item1sche1, item1type, '-', item2type, '-');
                } else if (item1size2 == item2size1) {
                    fill_txt(row, item1jtyp2, item1size2, item1sche2, item1type, '-', item2type, '-');
                }
                ...
                if (item1type == 'CAP') {
                    fill_txt(row, item2jtyp2, item2size1, item2sche1, item1type, '-', item2type, '-');
                } else {
                    fill_txt(row, item2jtyp1, item2size1, item2sche1, item1type, '-', item2type, '-');
                }
            } else {
                ...
                if (item1size2 == item2size1) {
                    fill_txt(row, item1jtyp2, item1size2, item1sche2, item1type, '-', item2type, '-');
                }
                if (item1size1 == item2size2) {
                    fill_txt(row, item1jtyp1, item1size1, item1sche1, item1type, '-', item2type, '-');
                }
            }
