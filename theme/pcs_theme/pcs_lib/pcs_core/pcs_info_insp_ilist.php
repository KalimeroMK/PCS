...
if ($insp_type == 'RT' || $insp_type == 'MT' || $insp_type == 'PT' || $insp_type == 'PAUT') {
    $insp_type = 'nde';
}
...
while ($sql_insp_dwg_arr = sql_fetch_array($sql_insp_dwg)) {
    echo '<input type="hidden" name="sel_dwg[]" value="' . $sql_insp_dwg_arr['dwg_no'] . '"/>';
}
...
?>
    <table class="main">
        <caption><a href='javascript:document.dwglist.submit()'><?php
                echo strtoupper($insp_type);
                ?> INSPECTION STATUS </a></caption>
        <tbody>
        <tr>
        ...
        while ($sql_insp_jnt_arr = sql_fetch_array($sql_insp_jnt)) {
            ...
            <tr>
                ...
                <td class="jnt_td"><a
                            href=<?php echo G5_URL . '/bbs/board.php?bo_table=iso&wr_id=' . $sql_g5_dwg_arr['wr_id']; ?> target='_self'>
                        <font style='font-size:25px;'> <b><?php echo $sql_insp_jnt_arr['dwg_no']; ?></b></font></a></td>

                <td class="jnt_td"><?php if ($sql_insp_jnt_arr['photo_1']) {
                        echo '<a onclick=\'window.open("' . $photo_path . '/' . $sql_insp_jnt_arr['photo_1'] . '.jpg","' . $sql_insp_jnt_arr['photo_1'] . '","width=650, height=500, left=200, top=100");\'><img src="' . $photo_path . '/' . $sql_insp_jnt_arr['photo_1'] . '.jpg" width="120px"><br>';
                    } ?></td>
                <td class="jnt_td"><?php if ($sql_insp_jnt_arr['photo_2']) {
                        echo '<a onclick=\'window.open("' . $photo_path . '/' . $sql_insp_jnt_arr['photo_2'] . '.jpg","' . $sql_insp_jnt_arr['photo_2'] . '","width=650, height=500, left=200, top=100");\'><img src="' . $photo_path . '/' . $sql_insp_jnt_arr['photo_2'] . '.jpg" width="120px"><br>';
                    } ?></td>
