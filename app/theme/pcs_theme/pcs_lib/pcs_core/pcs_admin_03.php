...
if(!$_POST['form_type']){
	switch($_GET['wr_id']){
		case 2 : $pcs_typ = 'joint';		$dir = PCS_REP_PDF;		break;
		case 3 : $pcs_typ = 'spool';		break;
		case 4 : $pcs_typ = 'iso';			$dir = PCS_DWG_ISO;		break;
		case 5 : $pcs_typ = 'pnid';			$dir = PCS_PNID_PDF;	break;
		case 6 : $pcs_typ = 'plan';			$dir = PCS_PLAN_PDF;	break;
		case 7 : $pcs_typ = 'tp';			break;
		case 8 : $pcs_typ = 'package';		break;
		case 9 : $pcs_typ = 'bmlist';		break;
		case 10 : $pcs_typ = 'itemcode';		break;
		case 11 : $pcs_typ = 'cutpipe';		break;
		case 12 : $pcs_typ = 'work';		break;
		default: break;
	}
...
Total <strong><?php echo $j_qty;?></strong> <?php echo $pcs_typ;?> data in PCS Database.<br>
If you upload Zip file, All <?php echo $pcs_typ;?> data <font color="red">will be updated!</font><br>
...
<input type="hidden" name="form_name" value="<?php echo $pcs_typ;?>">
<tr><td class="main_td" style="text-align:left; padding:10px;"><input type="file" name="zip_file" id='zip_file' style='width:100%' accept='application/x-zip-compressed' /></tr> 
<tr><td class="main_td" style="padding:10px;"><input type="submit" value="<?php echo ' '.$pcs_typ.'.zip';?> update " accesskey="s" class="btn_submit" /></tr> 
</form>
</tbody>
</table> 
</div>
...
<tr> 
<td class="main_td" style="text-align:left; padding:10px;">
Total <strong><?php echo count_files($dir);?></strong> PDF files in server.<br>
</td>
</tr>
<form name="pdffiles" id="pdffiles" method="post" enctype="multipart/form-data">
<input type="hidden" name="form_type" value="pdf">
<input type="hidden" name="form_name" value="<?php echo $pcs_typ;?>">
<tr><td class="main_td" style="text-align:left; padding:10px;"><input type="file" name="pdf_file[]" id='pdf_file' style='width:100%' accept='application/pdf' multiple /></tr> 
<tr><td class="main_td" style="padding:10px;"><input type="submit" value=" PDF upload " accesskey="s" class="btn_submit" onclick="submitAction();" /></tr> 
</form>
</tbody>
</table> 
</div>
		...
		if($_FILES['zip_file']['type']='application/x-zip-compressed') {
			if($_FILES['zip_file']['size']<5000000){
				if(move_uploaded_file($_FILES['zip_file']['tmp_name'], $dir.$zip_file_name)) {
					...
					else {sql_query ('TRUNCATE TABLE '.G5_TABLE_PREFIX.'write_'.$_POST['form_name']);}
					...
					for($j=1;$j<$lpcnt;$j++){
						...
						else if($_POST['form_name']=='spool'||$_POST['form_name']=='pnid'||$_POST['form_name']=='plan'||$_POST['form_name']=='tp'||$_POST['form_name']=='package'||$_POST['form_name']=='work'){
					...
					else{sql_query('UPDATE '.G5_TABLE_PREFIX.'board set bo_count_write = '.($j-1).' WHERE bo_table = "'.$_POST['form_name'].'"');}
				...
				else {echo "Error Update Data <br/>";}
			
			}
			else {echo 'Not allowed <b>over 5 Mb</b> zip file. <br/>';}
		}
		else {echo 'Not Zip file <br/>';}
	}
	else {
...
if($_POST['form_type']=='pdf'){
	...
	echo '<div style="font-size:30px; padding:30px;">';
	if($_POST['form_name']=='joint')	{$dir = PCS_REP_PDF.'/';	}
	if($_POST['form_name']=='iso')	{$dir = PCS_DWG_ISO.'/';	$fld = 'dwg_no';}
	if($_POST['form_name']=='pnid')		{$dir = PCS_PNID_PDF.'/';	$fld = 'pnid_no';}
	if($_POST['form_name']=='plan')		{$dir = PCS_PLAN_PDF.'/';	$fld = 'plan_no';}
	if($_POST['form_name']=='equipment'){$dir = PCS_EQUI_PDF.'/';	$fld = 'eq_no';}
	...
	for($i=0; $i<$temp_i; $i++){
		...
		if($_POST['form_name']!='joint')	{
			$query_file = 'SELECT '.$fld.' FROM '.G5_TABLE_PREFIX.'pcs_info_'.$_POST['form_name'].' WHERE '.$fld.' = "'.preg_replace('/_[0-9].pdf/', '', $pdf_file_name[$i]).'"';
...
//								echo ($i+1).'. '.$pdf_file_name[$i]." : <font color='blue'>Upload finished.</font> <br/>";
							...
							else {echo ($i+1).'. '.$pdf_file_name[$i].' : <font color="red">Error Update Data.</font> <br/>';}
						}
						else {echo ($i+1).'. '.$pdf_file_name[$i].' : <font color="red">Not allowed <b>over 5 Mb</b> pdf file.</font> <br/>';}
					}
					else {echo ($i+1).'. '.$pdf_file_name[$i].' :<font color="red"> Not pdf file.</font> <br/>';}
				}
				else {echo ($i+1).'. '.$pdf_file_name[$i].' : <font color="red">Error Update Data.</font> <br/>';}
			}
			else{echo ($i+1).'. '.$pdf_file_name[$i].' : <font color="red">Not in DATABASE.</font> <br/>';}
		}
		else{
				if(!$_FILES['pdf_file']['error'][$i]) {
					if($_FILES['pdf_file']['type'][$i]=='application/pdf') {
