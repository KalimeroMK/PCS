...
class QRcode {
	 ...
	 */
	protected function encodeString($string): null {
		$this->dataStr = $string;
	 ...
	 */
	protected function encodeMask($mask): null {
	 ...
	 */
	protected function setFrameAt(array $at, $val) {
	 ...
	 */
	protected function init($spec): int {
	 ...
	 * @return int blacks
	 */
	 protected function writeFormatInformation($width, array &$frame, $mask, $level): int {
		$blacks = 0;
	 ...
	 * @return int mask
	 */
	 protected function mask0($x, $y): int {
		return ($x + $y) & 1;
	}
	 ...
	 * @return int mask
	 */
	 protected function mask1($x, $y): int {
		return ($y & 1);
	}
	 ...
	 * @return int mask
	 */
	 protected function mask2($x, $y): int {
		return ($x % 3);
	}
	 ...
	 * @return int mask
	 */
	 protected function mask3($x, $y): int {
		return ($x + $y) % 3;
	}
	 ...
	 * @return int mask
	 */
	 protected function mask4($x, $y): int {
	 ...
	 * @return int mask
	 */
	 protected function mask5($x, $y): int {
		return (($x * $y) & 1) + ($x * $y) % 3;
	}
	 ...
	 * @return int mask
	 */
	 protected function mask6($x, $y): int {
	 ...
	 * @return int mask
	 */
	 protected function mask7($x, $y): int {
	 ...
	 */
	protected function generateMaskNo(string $maskNo, $width, $frame): array {
	 ...
	 * @return int demerit
	 */
	 protected function calcN1N3($length): int|float {
		$demerit = 0;
		for ($i=0; $i<$length; ++$i) {
	 ...
	 * @return int demerit
	 */
	 protected function evaluateSymbol($width, $frame): int|float {
		$head = 0;
		$demerit = 0;
		for ($y=0; $y<$width; ++$y) {
			$head = 0;
	 ...
	 */
	 protected function isdigitat(array $str, $pos) {
		if ($pos >= strlen($str)) {
			return false;
		}
	 ...
	 * @return boolean true of false
	 */
	 protected function isalnumat(array $str, $pos) {
		if ($pos >= strlen($str)) {
			return false;
		}
	 ...
	 * @return int mode
	 */
	 protected function identifyMode($pos): int {
		...
		while (strlen($this->dataStr) > 0) {
			...
			switch ($mode) {
				...
				case QR_MODE_AN:
                    ...
                    break;
				case QR_MODE_KJ:
                    $length = $hint == QR_MODE_KJ ? $this->eatKanji() : $this->eat8();
                    break;
				default:
                    $length = $this->eat8();
                    break;
	 ...
	 */
	 protected function newInputItem($mode, $size, $data, $bstream=null): ?array {
	 ...
	 * @return array input item
	 */
	 protected function encodeModeNum(array $inputitem, $version): array {
	 ...
	 * @return array input item
	 */
	 protected function encodeModeAn(array $inputitem, $version): array {
	 ...
	 * @return array input item
	 */
	 protected function encodeMode8(array $inputitem, $version): array {
		$inputitem['bstream'] = array();
	 ...
	 * @return array input item
	 */
	 protected function encodeModeKanji(array $inputitem, $version): array {
		$inputitem['bstream'] = array();
	 ...
	 * @return int parity
	 */
	 protected function calcParity($items): int {
		$parity = 0;
	 ...
	 */
	 protected function checkModeNum($size, $data): bool {
		for ($i=0; $i<$size; ++$i) {
	 ...
	 * @return boolean true or false
	 */
	 protected function checkModeAn($size, $data): bool {
		for ($i=0; $i<$size; ++$i) {
	 ...
	 */
	 protected function estimateBitsModeNum($size): int {
		$w = (int)($size / 3);
		$bits = ($w * 10);
		switch($size - ($w * 3)) {
			case 1:
	 ...
	 * @return int number of bits
	 */
	 protected function estimateBitsModeAn($size): int {
	 ...
	 * @return int number of bits
	 */
	 protected function estimateBitsMode8($size): int {
		return (int)($size * 8);
	}
	 ...
	 * @return int number of bits
	 */
	 protected function estimateBitsModeKanji($size): int {
	 ...
	 * @return boolean true or false
	 */
	 protected function checkModeKanji($size, $data): bool {
		if (($size & 1) !== 0) {
			return false;
		}
		for ($i=0; $i<$size; $i+=2) {
	...
	protected function check($mode, $size, $data) {
		...
		switch($mode) {
			case QR_MODE_NM: return $this->checkModeNum($size, $data);
			case QR_MODE_AN: return $this->checkModeAn($size, $data);
			case QR_MODE_KJ: return $this->checkModeKanji($size, $data);
			case QR_MODE_8B:
			case QR_MODE_ST: return true;
			default: break;
		}
		return false;
	}
	 ...
	 * @return int bits
	 */
	 protected function estimateBitStreamSize($items, $version): int|float {
		$bits = 0;
		if ($version == 0) {
			$version = 1;
		}
		foreach ($items as $item) {
			switch($item['mode']) {
				...
				case QR_MODE_KJ:
                    ...
                    break;
				case QR_MODE_ST: return STRUCTURE_HEADER_BITS;
				default: return 0;
			}
	...
	// - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      */
     protected function allocate($setLength): array {
	 ...
	 */
	 protected function bitstreamToByte($bstream): ?array {
		if (is_null($bstream)) {
	 		return null;
	 	}
		$size = count($bstream);
	 ...
	 * @return array srctab
	 */
	 protected function qrstrset(array $srctab, $x, $y, $repl, $replLen=false): array {
	 ...
	 */
	protected function getDataLength($version, $level): int|float {
	 ...
	 */
	protected function getMinimumVersion($size, $level): int {
	 ...
	 */
	protected function maximumWords($mode, $version): int|float {
		if ($mode == QR_MODE_ST) {
			return 3;
		}
		if ($version <= 9) {
	 ...
	 * @return int value
	 */
	 protected function rsBlockNum($spec): float|int|array {
	...
	* Return block number 1
	 ...
	 * @return int value
	 */
	 protected function rsDataLength($spec): int|float {
	 ...
	 * @return int value
	 */
	 protected function rsEccLength($spec): int|float {
	 ...
	 */
	 protected function modnn(array $rs, $x) {
		while ($x >= $rs['nn']) {
			$x -= $rs['nn'];
			$x = ($x >> $rs['mm']) + ($x & $rs['nn']);
		}
	 ...
	 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn = Symbols per block;</li><li>alpha_to = log lookup table array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly = Generator polynomial array;</li><li>nroots = Number of generator;</li><li>roots = number of parity symbols;</li><li>fcr = First consecutive root, index form;</li><li>prim = Primitive element, index form;</li><li>iprim = prim-th root of 1, index form;</li><li>pad = Padding bytes in shortened block;</li><li>gfpoly</ul>.
	 */
	protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $nroots, $pad): ?array {
	 ...
	 * @return parity array
	 */
	 protected function encode_rs_char(array $rs, $data, array $parity): array {
		...
		for ($i=0; $i < ($NN - $NROOTS - $PAD); $i++) {
			...
			// Shift
			array_shift($parity);
			$parity[] = $feedback != $A0 ? $ALPHA_TO[$this->modnn($rs, $feedback + $GENPOLY[0])] : 0;
		}
		return $parity;
	}
