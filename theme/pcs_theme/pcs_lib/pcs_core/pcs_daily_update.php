...
if ($_POST['issue_mode'] == 'issue') {
    ...
    if ($member['mb_7'] < 2 && $member['mb_nick'] != $sql_issue_cont_arr['wr_2'] && substr($sql_issue_cont_arr['pcs_vi_rlt_by'], 0, 10) != G5_TIME_YMD) {
        $rmv_ok = false;
    } else {
        $rmv_ok = true;
    }
...
<form name='daily' method="post" onSubmit="return doSumbit()">
    ...
    <table class="main" style="width:100%">
        ...
        <tr>
            ...
            <td class="main_td">
                <?php
                if ($_POST['issue_mode'] == 'issue' || $_POST['issue_mode'] == 'u_desc') {
                    echo '<input type="text" name="issue_desc" id="istxt" autocomplete="off" style="padding:0px 0px 0px 15px; text-align:left;width:98%;height:250px;font-size:20px;border:none;" onchange="reveal();" value="' . $_POST['wr_content'] . '">';
                } else {
                    echo $sql_issue_cont_arr['punch_desc'];
                }
                ?>
            </td>
            <td class="main_td" style="width:25%"><img id='pcs_prev1' width='350px'
                                                       src='<?php if ($sql_issue_cont_arr['issued_by']) {
                                                           echo $pncURL . '_BF.jpg';
        ...
        <tr>
            <td class="main_td" style="height:60px" colspan="2"><a id="sbm"
                                                                   href='javascript:document.daily.submit()' <?php if ($_POST['issue_mode'] != 'remove') {
                    echo 'hidden';
                } ?>>
                    <?php
                    if ($member['mb_7']) {
