...
//		unlink(PCS_DWG_FAB.'/fab_'.$_POST['mapped_dwg'].'_'.$_POST['rev_dwg'].'.pdf');
                    ...
                    if ($jnt_each_arr[6] != 'comment' && $jnt_each_arr[6] != 'coupling') {
                        ...
                        switch ($jnt_each_arr[6]) {
                            case 'bolt'    :
                                $j_type_sbc = 'BT';
                                $w_type_sbc = 'OTHER';
                                $jnt_sbc = $jnt_each_arr[10] * 1 < 10 ? 'B0' . $jnt_each_arr[10] : 'B' . $jnt_each_arr[10];;
                                break;
                            case 'thread'    :
                                $j_type_sbc = 'TH';
                                $w_type_sbc = 'OTHER';
                                $jnt_sbc = $jnt_each_arr[10] * 1 < 10 ? 'T0' . $jnt_each_arr[10] : 'T' . $jnt_each_arr[10];;
                                break;
                            case 'support'    :
                                $j_type_sbc = 'PS';
                                $w_type_sbc = 'OTHER';
                                $jnt_sbc = $jnt_each_arr[10] * 1 < 10 ? 'S0' . $jnt_each_arr[10] : 'S' . $jnt_each_arr[10];;
                                break;
                            case 'spool'    :
                                $j_type_sbc = 'SPL';
                                $w_type_sbc = 'OTHER';
                                $jnt_sbc = $jnt_each_arr[10] * 1 < 10 ? 'SP0' . $jnt_each_arr[10] : 'SP' . $jnt_each_arr[10];;
                                break;
                            default            :
                                $j_type_sbc = 'after';
                                $w_type_sbc = 'WELD';
                                $pmi_yn = $sql_dwg_array['pmi'];
                                $pwht_yn = $sql_dwg_array['pwht'];
                                ...
                                if ($jnt_each_arr[11] * 1) {
                                    ...
                                    $jnt_sbc = $jnt_each_arr[10];
                                }

                                break;
                        }

                        if ($jnt_each_arr[6] == 'shop') {
                            $s_f = 'S';
                            $j_type_sbc = 'WJ';
                            $spn = $jnt_each_arr[15];
                        }
                        if ($jnt_each_arr[6] == 'field') {
                            $s_f = 'F';
                            $j_type_sbc = 'WJ';
                            $spn = '';
                        };
                        if ($jnt_each_arr[6] == 'spool') {
                            $spn = $jnt_each_arr[5] . '-' . $jnt_sbc;
                        };
                        ...
                        if (sql_fetch_array($sql_query_check)) {
                            $query_jnt_sbc = 'UPDATE markDwgJoint SET 
												...
												WHERE j_key = "' . $_POST['mapped_dwg'] . '_' . $jnt_sbc . '"';
                            sql_query($query_jnt_sbc);
                            $query_dwg_sta = 'UPDATE markDwgCoor SET
													dwg_state = "Marked" 
												WHERE rev_no = "' . $_POST['rev_dwg'] . '"';
                            ...
                            $query_jnt_sbc = 'INSERT INTO markDwgJoint SET
													...
													pmi_yn = "' . $pmi_yn . '" ,
													pwht_yn = "' . $pwht_yn . '" ,
													nde_rate = "' . $sql_dwg_array['nde_rate'] . '"';
                            sql_query($query_jnt_sbc);
    ...
    <div style='position: fixed; top:0; left:0; height:45px; width:100%; border:2px solid green; font-size:15px; align:center; display:-webkit-flex; display:flex; -webkit-align-items:center; align-items: center;'>
        <input type='radio' name='joint_type' style="height:20px; width: 25px;" onclick="chk_radio('0')" value='spool'>
        SPOOL no. :
        <input type='text' id='spl_dwg' style='height:30px; width:300px; font-size:20px; padding:5px;'> <input
                type='text' id='spl_no' style='height:30px; width:50px; font-size:20px; padding:5px;'>&nbsp; &nbsp;
        Joint no. : <input type='text' id='jnt_no' style='height:30px; width:50px; font-size:25px; padding:5px;'
                           disabled>
        <input type='radio' name='joint_type' style="height:20px; width: 25px;" onclick="chk_radio('1')" value='shop'
               checked>WELD(shop)
        ...
        <input type='radio' name='joint_type' style="height:20px; width: 25px;" onclick="chk_radio('4')"
               value='support'>SUPPORT
        <input type='radio' name='joint_type' style="height:20px; width: 25px;" onclick="chk_radio('5')" value='thread'>THREAD
        <input type='radio' name='joint_type' style="height:20px; width: 25px;" onclick="chk_radio('6')"
               value='coupling' disabled>COUPLING
        <input type='radio' name='joint_type' style="height:20px; width: 25px;" onclick="chk_radio('7')" value='comment'
               disabled>Comment :
        <input type='text' id='com_info' style='height:30px; width:200px; font-size:25px; padding:5px;' disabled> &nbsp;
        &nbsp;
        <input type='button' id='undo' value='Undo' style="height:30px; width:100px; font-size:16px;"
               onclick='undoJointmapping()'>
    ...
    <div style='width:350px; position: absolute; top: 49px; right: 0px ; height:95%;'>
        <div style='width:99%; height:20%; border:1px solid green; font-size:20px;'>
            ...
            &nbsp; <input type='text' id='joint_line' style='height:35px; width:60px; font-size:25px; padding:10px;'
                          value='<?php echo $sql_dwg_spec_array['linewidth']; ?>'> : Linewidth <br>
            &nbsp; <input type='text' id='joint_size' style='height:35px; width:60px; font-size:25px; padding:10px;'
                          value='<?php echo $sql_dwg_spec_array['fontsize']; ?>'> : Size <br>
            ...
            <form name='num_smt' method="post">
                ...
                <p align="center">
                    ...
                    <input type="submit" id="n_finish" value="FINISH"
                           style="height:35px; width:100px; font-size:18px; background-color:greenyellow;">


                    &nbsp;&nbsp;

                    <?php if (!$joint_coor_curr && $joint_coor_prev) { ?>
                        <input type="button" id="j_follow" name="j_follow" value="INHERIT"
                               style="height:35px; width:100px; font-size:18px; background-color:yellow;"
                               onclick="return j_follow_onclick()"/>
                        <input type='hidden' id='jnt_follow' name='jnt_follow' value='jnt_follow'>
                    <?php } else { ?>

                        <input type="button" id="allClear" name="allClear" value="CLEAR"
                               style="height:35px; width:100px; font-size:18px; background-color:red;"
        ...
        <div style='overflow:scroll; width:99%; height:54%; border:1px solid red;'>
            <table id='jnt_table'
                ...
                <tr style="height: 30px; background-color: aquamarine;">
                    ...
                    <td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 80px"><p
                                align="center">S/F</p></td>
                    <td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 80px"><p
                                align="center">State</p></td>
                    <td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 80px"><p
                                align="center">Spool</p></td>
                </tr>
    ...
    <script>
        ...
        $(txb_spn).focusin(function () {
            txb_spd.disabled = false;
        });
        ...
        var jntxt = 0;
        var jntyt = 0;
        var jntMarkType = 'shop';
        var dwgNo = '<?php echo $dwg_file;?>';
        var revNo = '<?php echo $latest_rev;?>';
        var jntLineWidth = document.getElementById('joint_line').value * 1;
        var jntSize = document.getElementById('joint_size').value * 1;
        ...
        txb_spd.value = dwgNo;
        var tmptf = 'Tmp';
        var rec_Jinfo = '<?php echo $joint_coor_curr;?>';
            ...
            for (var i = 0; i < jointArray.length - 1; i++) {
                ...
                switch (jointData[6]) {
                    case 'shop' :
                        ...
                        break;
                    case 'field' :
                        radio_btn[2].checked = true;
                        break;
                    case 'bolt' :
                        radio_btn[3].checked = true;
                        break;
                    case 'support':
                        radio_btn[4].checked = true;
                        break;
                    case 'thread' :
                        radio_btn[5].checked = true;
                        break;
                    case 'coupling' :
                        radio_btn[6].checked = true;
                        break;
                    case 'comment' :
                        radio_btn[7].checked = true;
                        document.getElementById('com_info').value = jointData[10];
                        break;
                    case 'spool' :
                ...
                if (jointData[11] * 1) {
                    txb_his.value = jointData[10]
                }
                ;
                if (dwgNo != jointData[5]) {
                    txb_spd.value = jointData[5];
                    txb_spn.value = jointData[10];
                } else {
                    txb_spd.value = jointData[5];
                    txb_spn.value = ''
                }
...
//	splAd = txb_spn.value;
        ...
        function SF_toggle(type) {
            ...
            if (type == 'shop') {
                radio_obj[1].checked = true;
            } else if (type == 'field') {
                radio_obj[2].checked = true;
            }
        }
            ...
            for (var i = 0; i < radio_btn.length; i++) {
                if (radio_btn[i].checked) {
                    ...
                    switch (jntMarkType) {
                        case 'comment' :
                            jntPre = 'CMT';
                            jntNo = document.getElementById('com_info').value;
                            break;
                        case 'coupling' :
                            jntPre = 'CPL';
                            jntNo = 'CPL';
                        ...
                        case 'spool' :
                            ...
                            if (dwgNo != txb_spd.value) {
                                ...
                                countspool++;
                            } else if (inde == 'decr' && countspool != 0) {
                                countspool--;
                            }
                            ;
                            jntNo = countspool;
                        ...
                        case 'thread' :
                            ...
                            if (inde == 'incr') {
                                countthread++;
                            } else if (inde == 'decr' && countthread != 0) {
                                countthread--;
                            }
                            ;jntNo = countthread;
                            break;
                        case 'support' :
                            ...
                            if (inde == 'incr') {
                                countsupport++;
                            } else if (inde == 'decr' && countsupport != 0) {
                                countsupport--;
                            }
                            ;jntNo = countsupport;
                            break;
...
//	minimap();
        ...
        function jointDataPush() {
            ...
            if (jntAd) {
                addValue[Sno] = Sno + ',' + jntxf + ',' + jntyf + ',' + jntxt + ',' + jntyt + ',' + txb_spd.value + ',' + jntMarkType + ',' + jntLineWidth + ',' + jntSize + ',' + jntPre + ',' + jntAd + ',1,ACT,,' + jntAd + ',,;';
            } else {
                addValue[Sno] = Sno + ',' + jntxf + ',' + jntyf + ',' + jntxt + ',' + jntyt + ',' + txb_spd.value + ',' + jntMarkType + ',' + jntLineWidth + ',' + jntSize + ',' + jntPre + ',' + jntNo + ',0,ACT,,' + cob_jn + ',,;';
            }
        ...
        function addRowInTable(jntState, itm1, itm2, spl1) {
            ...
            if (jntMarkType == 'spool') {
                txbfocus = 'df';
            }

            newTr = jntTable.insertRow(rowIndex);
            newTr.align = 'center';
            if (jntState == 'DEL' || jntState == 'REM') {
            ...
            if (jntMarkType == 'spool' && (dwgNo != txb_spd.value)) {
                jono = txb_spn.value;
            } else {
                jono = jntNo;
            }
            ...
            if (jntAd) {
                intxt = jntAd;
            } else if (jntMarkType == 'comment') {
                intxt = jntNo;
            } else if ((jntMarkType == 'shop' || jntMarkType == 'field') && jono * 1 < 1000) {
                intxt = jono;
            } else if (jntMarkType == 'spool' && jntNo * 1 < 10) {
                intxt = jntPre + '0' + jono;
            } else {
                intxt = jntPre + jono;
            }
            ...
            if (jntMarkType == 'spool' && jntNo == 'R') {
                intxt = '(PIPE)';
            }
            newTd1.innerHTML = intxt;
            $(newTd1).on("click", newTd1, function () {
                if (txbfocus && fromtxb) {
                    onclickfillspl(fromtxb, (rowIndex - 1));
                }
                fromtxb = '';
            });
            ...
            $(newTd2).on("click", newTd2, function () {
                ...
                if (jtp != 'BT' || jtp != 'PS' || jtp != 'SPL') {
                    SFChange(rowIndex - 1);
                    if ($(this).closest("tr").find("td:eq(1)").text() == 'S') {
                        $(this).closest("tr").find("td:eq(1)").text('F');
                    } else {
                        $(this).closest("tr").find("td:eq(1)").text('S');
                    }
                }
            });
            ...
            if (spl1 == '') {
                txbSpl.value = spl1;
            } else if (spl1.search(dwgNo) < 0) {
                txbSpl.value = '<' + right(spl1, 2) * 1 + '>';
            } else if (txbSpl.value = right(spl1, 2) * 1) {
                txbSpl.value = right(spl1, 2) * 1;
            }

            if (jntMarkType == 'shop') {
                if (!txbSpl.value) {
                    $(txbSpl).css('background-color', 'red');
                }
                ;
                newTd4.appendChild(txbSpl);
                $(txbSpl).focusin(function () {
                    txbSpl.value = '';
                    $(txbSpl).css('background-color', 'white');
                    fromtxb = rowIndex - 1;
                });
                ...
                txbSpl.value = 'N/A';
                txbSpl.disabled = true;
                newTd4.appendChild(txbSpl);
                $(txbSpl).css('background-color', 'gray');
            }
        }
        ...
        function drawJoint(xf, yf, xt, yt, dn, j_type, l, s, pr, no, ad, T_F) {
            ...
            if (T_F) {
                ...
                if (ad * 1) {
                    var jointNo = no;
                } else if ((jnt_type == 'shop' || jnt_type == 'field') && no * 1 < 1000) {
                    jointNo = no;
                }
...
//		var NoY = yt + textSize/3;
        ...
        function mPush(evt, currentObj) {
            ...
            canvas.addEventListener('mousemove', function (evt) {
                ...
                if (document.getElementById('com_info').value && jntPre == 'CMT') {
                    jn = document.getElementById('com_info').value;
                }
                if (event.button == '0') {
                    ...
                    ;
                    if (txb_spn.value.toUpperCase() == 'R') {
                        jntNo = 'R';
                    }
                    ;
