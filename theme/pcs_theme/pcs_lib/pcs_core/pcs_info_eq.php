...
<table class="main">
    ...
    <tbody>
    <tr>
        <td class="main_td td_sub" style="height:80px;" colspan="6">
            <b> <?php echo $sql_equipment_arr['description']; ?> </b></td>

    </tr>
    <tr>
        <td class="main_td" style="height:80px;"> <?php pdf_v(1, $sql_equipment_arr['dwg_1'], 'Foundation'); ?></td>
        <td class="main_td"
            style="height:80px; font-size:25px;"> <?php pdf_v(2, $sql_equipment_arr['dwg_2'], 'General Arrangement'); ?></td>
    ...
    <tr>
        ...
        if ($sqlISO) {
            ...
            while ($sqlISO_arr = sql_fetch_array($sqlISO)) {
                ...
                for ($noz = 0; $noz < count($noz_arr) - 1; $noz++) {
                    $eq_nozzle = str_replace($view['wr_subject'] . '_', "", $noz_arr[$noz]);
                }

                if ($sql_con_dwg_arr['wr_id']) {
                    echo '<a href=' . G5_BBS_URL . '/board.php?bo_table=drawing&wr_id=' . $sql_con_dwg_arr['wr_id'] . '> <b>' . $eq_nozzle . ' : ' . $sqlISO_arr['dwg_no'] . '</b></a></td>';
                } else {
                    echo '<mark>' . $sqlISO_arr['dwg_no'] . '</mark></td>';
                }
