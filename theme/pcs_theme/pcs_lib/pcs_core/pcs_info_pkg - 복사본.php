...
if ($_POST['pkg_cancle']) {
    $query_step = 'UPDATE ' . G5_TABLE_PREFIX . 'pcs_info_pkg_stat SET ' . $_POST['pkg_cancle'] . ' = "0000-00-00" WHERE pkg_no = "' . $view['wr_subject'] . '"';
    sql_query($query_step);
}
if ($_POST['pkg_step']) {
    $query_step = 'UPDATE ' . G5_TABLE_PREFIX . 'pcs_info_pkg_stat SET ' . $_POST['pkg_step'] . ' = "' . G5_TIME_YMD . '" WHERE pkg_no = "' . $view['wr_subject'] . '"';
    sql_query($query_step);
}
...
$Apunch_issue = $gen_Apunch_issue;
$Apunch_clear = $gen_Apunch_clear;
$Bpunch_issue = $gen_Bpunch_issue;
$Bpunch_clear = $gen_Bpunch_clear;
$Cpunch_issue = $gen_Cpunch_issue;
$Cpunch_clear = $gen_Cpunch_clear;
    ...
    <div style="overflow:hidden;height:auto;">
        ...
        <table class="main">
            ...
            while ($sql_ref_pnid_arr = sql_fetch_array($sql_ref_pnid)) {
                ...
                <tr>
                    ...
                    <td class="main_td"><a
                                href=<?php echo G5_URL . '/bbs/board.php?bo_table=pnid&wr_id=' . $sql_pkgby_pnid_ref_arr[wr_id]; ?> target='_self'><?php echo $sql_ref_pnid_arr['pnid_no']; ?></a>
                    </td>
                    <td class="main_td">
                        <a href='javascript:document.pnid<?php echo $pnidQty; ?>.submit()'><b>View</b></a>
                        <form name="pnid<?php echo $pnidQty; ?>"
                              action="<?php echo PCS_CORE_URL; ?>/pcs_mark_masterpnid.php" method="post"
        ...
        <table class="main">
            ...
            <tr>
				...
				';
                    ...
                    if ($sql_pkg_state_arr[$pkg_db[$i]] == '0000-00-00') {
                        echo 'Not yet';
                    } else {
                        echo $sql_pkg_state_arr[$pkg_db[$i]];
                    }
                    echo '</td>';
                }
        ...
        <p>&nbsp;
            ...
            while ($sql_pkg_stat_arr = sql_fetch_array($sql_pkg_stat)) {
                ...
                */
                ...
                $total_jnt += $sql_workvolumn_total_jnt;
                $total_DI += $sql_workvolumn_total_DI;
                $done_jnt += $sql_workvolumn_done_jnt;
                $done_DI += $sql_workvolumn_done_DI;
                $spt_total += $sql_workvolumn_total_spt;
                $spt_done += $sql_workvolumn_done_spt;
                $pwht_total += $sql_workvolumn_total_pwht;
                $pwht_done += $sql_workvolumn_done_pwht;
                $pmi_total += $sql_workvolumn_total_pmi;
                $pmi_done += $sql_workvolumn_done_pmi;
                ...
                $bw_f_total += $sql_f_bw;
                $rt_f_done += $sql_f_rt;
                $bw_total += $sql_s_bw + $sql_f_bw;
                $rt_done += $sql_s_rt + $sql_f_rt;
                ...
                $Apunch_issue += $sql_Apunch_issue;
                $Apunch_clear += $sql_Apunch_clear;
                $Bpunch_issue += $sql_Bpunch_issue;
                $Bpunch_clear += $sql_Bpunch_clear;
                $Cpunch_issue += $sql_Cpunch_issue;
                $Cpunch_clear += $sql_Cpunch_clear;
                ...
                $pkg_table_array[$i][5] = $sql_pkg_coor_array['tb_qty'] ? $sql_pkg_coor_array['tb_qty'] : '-';
                ...
                for ($j = 0; $j < $temp_j; $j++) {
                    ...
                    if ($pkg_jnt_arr[5] == 'page' && $pkg_jnt_arr[8] == 'Act') {
                        $count_pageNo++;
                        $pkg_table_array[$i][6][$count_pageNo] = $pkg_jnt_arr[7] * 1;
                    }
                    if ($pkg_jnt_arr[5] == 'mark' && $pkg_jnt_arr[8] == 'Act') {
                        $count_markNo++;
                        $pkg_table_array[$i][7][$count_markNo] = $pkg_jnt_arr[7];
                    }
                }
                ...
                if (($sql_workvolumn_total_jnt) == 0) {
                    ...
                    $pkg_table_array[$i][8][0] = $sql_workvolumn_done_jnt == $sql_workvolumn_total_jnt ? 0 : 1;
                ...
                if (($sql_workvolumn_total_spt) == 0) {
                    ...
                    $pkg_table_array[$i][9][0] = $sql_workvolumn_done_spt == $sql_workvolumn_total_spt ? 0 : 1;
                ...
                if (($sql_workvolumn_total_pmi) == 0) {
                    ...
                    $pkg_table_array[$i][10][0] = $sql_workvolumn_done_pmi == $sql_workvolumn_total_pmi ? 0 : 1;
                ...
                if (($sql_workvolumn_total_pwht) == 0) {
                    ...
                    $pkg_table_array[$i][11][0] = $sql_workvolumn_done_pwht == $sql_workvolumn_total_pwht ? 0 : 1;
                    $pkg_table_array[$i][11][1] = ($sql_workvolumn_done_pwht) . ' / ' . ($sql_workvolumn_total_pwht);
                }

                if (($sql_s_bw + sql_f_bw) == 0) {
                    $pkg_table_array[$i][12] = '-';
                } else {
                    $pkg_table_array[$i][12] = $sql_s_bw == 0 ? '-' : 'S : ' . ($sql_s_rt) . ' / ' . ($sql_s_bw);
                    if ($sql_f_bw == 0) {
                        $pkg_table_array[$i][12] .= '<br>-';
                    } else {
                        $pkg_table_array[$i][12] .= '<br>F : ' . ($sql_f_rt) . ' / ' . ($sql_f_bw);
                    }
                }

                if (($sql_s_bw + sql_f_bw) == 0) {
                    $pkg_table_array[$i][13] = 'N/A';
                } else {
                    $pkg_table_array[$i][13] = sprintf("%2.0f", ($sql_s_rt + $sql_f_rt) / ($sql_s_bw + $sql_f_bw) * 100) . ' %';
                }

                if (($sql_Apunch_issue) == 0) {
                    $pkg_table_array[$i][15][0] = '-';
                    $pkg_table_array[$i][15][1] = false;
                } else {
                    $pkg_table_array[$i][15][0] = ($sql_Apunch_clear) . ' / ' . ($sql_Apunch_issue);
                    $pkg_table_array[$i][15][1] = $sql_Apunch_clear != $sql_Apunch_issue;
                }
                if (($sql_Bpunch_issue) == 0) {
                    $pkg_table_array[$i][16][0] = '-';
                    $pkg_table_array[$i][16][1] = false;
                } else {
                    $pkg_table_array[$i][16][0] = ($sql_Bpunch_clear) . ' / ' . ($sql_Bpunch_issue);
                    $pkg_table_array[$i][16][1] = $sql_Bpunch_clear != $sql_Bpunch_issue;
                }
                if (($sql_Cpunch_issue) == 0) {
                    $pkg_table_array[$i][17][0] = '-';
                    $pkg_table_array[$i][17][1] = false;
                } else {
                    $pkg_table_array[$i][17][0] = ($sql_Cpunch_clear) . ' / ' . ($sql_Cpunch_issue);
                    $pkg_table_array[$i][17][1] = $sql_Cpunch_clear != $sql_Cpunch_issue;
                }


                $pkg_table_array[$i][18] = $sql_pkgby_dwg_ref_arr['wr_id'];
                $pkg_table_array[$i][19] = $tb_sum;
                $tb_sum += $sql_pkg_coor_array['tb_qty'];
                $pkg_table_array[$i][20] = $sql_pkg_coor_array['dwg_state'];
                $pkg_finish = $sql_pkg_coor_array['dwg_state'] ? $pkg_finish : $pkg_finish * 0;
            ...
            for ($i = 0; $i < $k; $i++) {
                ...
                for ($l = 0; $l < $temp_l; $l++) {
                    ...
                    for ($m = 0; $m < $tmp_m; $m++) {
                        if ($pkg_table_array[$tmp_x][6][$m] == $i + 1) {
                            $pkg_page_no[$i][$l] = 1;
                            break;
                        } else {
                            $pkg_page_no[$i][$l] = 0;
            ...
            $G_total_A = $Apunch_issue == 0 ? '-' : $Apunch_clear . ' / ' . $Apunch_issue;
            $G_total_B = $Bpunch_issue == 0 ? '-' : $Bpunch_clear . ' / ' . $Bpunch_issue;
            $G_total_C = $Cpunch_issue == 0 ? '-' : $Cpunch_clear . ' / ' . $Cpunch_issue;
            $G_total_jnt = $total_jnt == 0 ? 'N/A' : $done_jnt . ' / ' . $total_jnt;
            $G_total_supt = $spt_total == 0 ? 'N/A' : $spt_done . ' / ' . $spt_total;
            $G_total_PWHT = $pwht_total == 0 ? 'N/A' : $pwht_done . ' / ' . $pwht_total;
            $G_total_PMI = $pmi_total == 0 ? 'N/A' : $pmi_done . ' / ' . $pmi_total;
        ...
        <table class="main">
            ...
            <tr>
                ...
                <td class="jnt_td jnt_th" style="width:7%" rowspan="2"> T.P</td>
            ...
            <tr>
                ...
                <?php if ($pmi_total == 0) {
                    ...
                    <?php if ($pmi_total == 0) {
                        ...
                        <?php if ($pmi_done == $pmi_total) {
                            echo $pmi_done . ' / ' . $pmi_total;
                        } else {
                            echo $G_total_PMI;
                        }
                    } ?></td>
                ...
                <?php if ($pwht_total == 0) {
                    ...
                    <?php if ($pwht_total == 0) {
                        ...
                        <?php if ($pwht_done == $pwht_total) {
                            echo $pwht_done . ' / ' . $pwht_total;
                        } else {
                            echo $G_total_PWHT;
                        }
                    } ?></td>

                <td class="jnt_td" style="background-color:<?php if ($done_jnt == $total_jnt) {
                    echo 'gold';
                } else {
                    echo 'red';
                } ?>;">
                    <?php if ($done_jnt == $total_jnt) {
                        echo $done_jnt . ' / ' . $total_jnt;
                    } else { ?>
                        <?php echo $G_total_jnt . '<br>' . $done_DI;
                    } ?></td>
                ...
                <?php if ($spt_total == 0) {
                    ...
                    <?php if ($spt_total == 0) {
                        echo 'N/A';
                    } elseif ($spt_done == $spt_total) {
                        echo $spt_done . ' / ' . $spt_total;
                    } else {
                        echo $G_total_supt;
                    } ?></td>
                ...
                <?php if ($bw_total == 0) {
                    echo 'gold';
                } elseif ($rt_done / $bw_total < $sql_ref_pkg_arr['nde'] / 100) {
                    echo 'red';
                } else {
                    echo 'gold';
                } ?>; font-size:12px" colspan=2>
                    <?php echo 'S : ' . $rt_s_done . ' / ' . $bw_s_total;
                    if ($bw_s_total) {
                        echo ' ( ' . sprintf("%2.1f", ($rt_s_done) / ($bw_s_total) * 100) . ' % )<br>';
                    } else {
                        echo ' ( 0.0 %)<br>';
                    } ?>
                    <?php echo 'F : ' . $rt_f_done . ' / ' . $bw_f_total;
                    if ($bw_f_total) {
                        echo ' ( ' . sprintf("%2.1f", ($rt_f_done) / ($bw_f_total) * 100) . ' % )<br>';
                    } else {
                        echo ' ( 0.0 %)<br>';
                    } ?>
                    <?php echo 'T : ' . $rt_done . ' / ' . $bw_total;
                    if ($bw_total) {
                        echo ' ( ' . sprintf("%2.1f", ($rt_done) / ($bw_total) * 100) . ' % )';
                    } else {
                        echo ' ( 0.0 %)<br>';
                    } ?>
                </td>
                ...
                <td class="jnt_td" style="background-color:
                <?php if ($Apunch_issue == 0) {
                    echo 'gold';
                } elseif ($Apunch_clear / $Apunch_issue == 1) {
                    echo 'gold';
                } else {
                    echo 'red';
                } ?>;">
                ...
                <td class="jnt_td" style="background-color:
                <?php if ($Bpunch_issue == 0) {
                    echo 'gold';
                } elseif ($Bpunch_clear / $Bpunch_issue == 1) {
                    echo 'gold';
                } else {
                    echo 'red';
                } ?>;">
                ...
                <td class="jnt_td" style="background-color:
                <?php if ($Cpunch_issue == 0) {
                    echo 'gold';
                } elseif ($Cpunch_clear / $Cpunch_issue == 1) {
                    echo 'gold';
                } else {
                    echo 'red';
                } ?>;">
            ...
            <tr>
                ...
                <td class="jnt_td" style="text-align:center; font-size:25px;" colspan="6">
                    ...
                    if (1 !== 0) {
                        ...
                        <form name='submit_for_pkg' action="<?php echo PCS_CORE_URL; ?>/pcs_viewPKG_pdf2.php"
                              method="post" target="pkg" onSubmit="return doSumbit()">
                ...
                <td class="jnt_td">
                    <?php if ($member['mb_7'] > 0) { ?>
                        <a href='javascript:document.submit_punch_<?php echo $i; ?>.submit()'><img
                                    src="<?php echo PCS_CORE_URL; ?>/punch_icon.png" width="35px"></a>
                        <form name='submit_punch_<?php echo $i; ?>' method="post" target="_self"
                ...
                <td class="jnt_td" <?php if ($gen_Apunch_clear != $gen_Apunch_issue) {
                    echo 'style="background-color:red"';
                } ?>>    <?php if ($gen_Apunch_issue == 0) {
                        echo '-';
                    } else {
                        echo $gen_Apunch_clear . ' / ' . $gen_Apunch_issue;
                    } ?> </td>
                <td class="jnt_td" <?php if ($gen_Bpunch_clear != $gen_Bpunch_issue) {
                    echo 'style="background-color:red"';
                } ?>>    <?php if ($gen_Bpunch_issue == 0) {
                        echo '-';
                    } else {
                        echo $gen_Bpunch_clear . ' / ' . $gen_Bpunch_issue;
                    } ?> </td>
                <td class="jnt_td" <?php if ($gen_Cpunch_clear != $gen_Cpunch_issue) {
                    echo 'style="background-color:red"';
                } ?>>    <?php if ($gen_Cpunch_issue == 0) {
                        echo '-';
                    } else {
                        echo $gen_Cpunch_clear . ' / ' . $gen_Cpunch_issue;
                    } ?> </td>
            </tr>
            ...
            for ($i = 0; $i < $k; $i++) {
                ...
                <tr id='tr<?php echo $i; ?>'>
                    <td class="jnt_td"><a
                                href='javascript:document.marked<?php echo $pkg_table_array[$i][0]; ?>.submit()'><font
                                    style='font-size:20px;'><?php echo $pkg_table_array[$i][0]; ?> </font></a>
                        <?php viewPDF('marked' . $pkg_table_array[$i][0], '', $pkg_table_array[$i][1], $pkg_table_array[$i][2], $view['wr_subject']); ?>

                    </td>
                    <td class="jnt_td"><a
                                href=<?php echo G5_URL . '/bbs/board.php?bo_table=iso&wr_id=' . $pkg_table_array[$i][18]; ?> target='_self'><?php echo $pkg_table_array[$i][1]; ?></a><BR><b> <?php echo $pkg_table_array[$i][21]; ?></b>
                    </td>
                    ...
                    <td class="jnt_td">
                        ...
                        if ($pkg_table_array[$i][20] == 'Approved') {
                            $txt_color = 'green';
                            $txt_value = 'Marked';
                        } elseif ($pkg_table_array[$i][20] == 'Marked') {
                            $txt_color = 'blue';
                            $txt_value = 'Approved';
                        }
                        if ($member['mb_3'] == 3) {
		...
		<input type="hidden" name="html_loc" value="' . $i . '">
                    ...
                    <td class="jnt_td">
                        ...
                        if ($pkg_table_array[$i][20] != 'Approved' && $member['mb_3'] > 0) {
                            ?>
                            <a href='javascript:document.numbering_form_<?php echo $pkg_table_array[$i][0]; ?>.submit()'><font
                                        style='font-size:20px;'><b><?php echo $pkg_table_array[$i][3] . '"'; ?></a>
                            <form name="numbering_form_<?php echo $pkg_table_array[$i][0]; ?>"
                                  action="<?php echo PCS_CORE_URL; ?>/pcs_mark_pkg_pdf.php" method="post"
                                  target="_blank" onSubmit="return doSumbit()">
                    ...
                    <td class="jnt_td"> <?php if ($pkg_table_array[$i][5]) {
                            echo $pkg_table_array[$i][5];
                        } else {
                            echo '-';
                        } ?></td>
                    ...
                    <td class="jnt_td">
                        ...
                        for ($j = 0; $j < $tmp_count; $j++) {
                            if ($pkg_page_no[$i][$j] !== 0) {
                                echo $pkg_table_array[$i][6][$j] . ' ';
                            } else {
                                $idx_red = $pkg_table_array[$i][6][$j] - 1;
                                ?>
                                <form name="numbering_form2_<?php echo $pkg_table_array[$idx_red][0]; ?>"
                                      action="<?php echo PCS_CORE_URL; ?>/pcs_mark_pkg_pdf.php" method="post"
                                      target="result" onSubmit="return doSumbit()">
                                    <input type="hidden" name="key" value="key">
                                    <input type="hidden" name="fn" value="<?php echo $pkg_table_array[$idx_red][1]; ?>">
                                    <input type="hidden" name="rev"
                                           value="<?php echo $pkg_table_array[$idx_red][2]; ?>">
                                    <input type="hidden" name="pn" value="<?php echo $sql_ref_pkg_arr['pkg_no']; ?>">
                                    <input type="hidden" name="sn" value="<?php echo $pkg_table_array[$idx_red][0]; ?>">
                                    <input type="hidden" name="tb"
                                           value="<?php echo $pkg_table_array[$idx_red][19]; ?>">
                    ...
                    <td class="jnt_td">
                        <?php if ($pkg_table_array[$i][22] == 'S/S') {
                            echo '<font color = "blue"><b>' . $pkg_table_array[$i][22] . '</b></font>';
                        } elseif ($pkg_table_array[$i][22] == 'A/S') {
                            echo '<font color = "red"><b>' . $pkg_table_array[$i][22] . '</b></font>';
                        } else {
                            echo $pkg_table_array[$i][22];
                        }
                        ?>
                    </td>
                    ...
                    <td class="jnt_td" <?php if ($pkg_table_array[$i][8][0] !== 0) {
                        echo 'style="background-color:red"';
                    } ?>><?php echo $pkg_table_array[$i][8][1]; ?></td>

                    <td class="jnt_td" <?php if ($pkg_table_array[$i][9][0] !== 0) {
                        echo 'style="background-color:red"';
                    } ?>><?php echo $pkg_table_array[$i][9][1]; ?></td>
                    ...
                    <td class="jnt_td" <?php if ($pkg_table_array[$i][15][1]) {
                        echo 'style="background-color:red"';
                    } ?>><?php echo $pkg_table_array[$i][15][0]; ?></td>

                    <td class="jnt_td" <?php if ($pkg_table_array[$i][16][1]) {
                        echo 'style="background-color:red"';
                    } ?>><?php echo $pkg_table_array[$i][16][0]; ?></td>

                    <td class="jnt_td" <?php if ($pkg_table_array[$i][17][1]) {
                        echo 'style="background-color:red"';
                    } ?>><?php echo $pkg_table_array[$i][17][0]; ?></td>
...
//			$pkg_dwg = $sql_pkg_tbarray['dwg_no'];
                    ...
                    for ($j = 0; $j < $temp_j; $j++) {
                        ...
                        if ($pkg_jnt_arr[5] == 'tbno') {
                            ...
                            <tr>
                                ...
                                <td class="jnt_td" style="height:40px;"><?php if (($pkg_jnt_arr[7] + $old_qty) < 10) {
                                        echo 'TB-0' . ($pkg_jnt_arr[7] + $old_qty);
                                    } else {
                                        echo 'TB-' . ($pkg_jnt_arr[7] + $old_qty);
                                    }; ?></td>
            ...
            <table class="main">
                ...
                while ($sql_pkg_Pchk_arr = sql_fetch_array($sql_pkg_Pchk)) {
                    if ($sql_pkg_Pchk_arr['s_no'] * 1 < 10) {
                        $jpgfile = PCS_PKG_URL . '/' . $sql_pkg_Pchk_arr['pkg_no'] . '/' . $sql_pkg_Pchk_arr['pkg_no'] . '_00' . $sql_pkg_Pchk_arr['s_no'];
                    } elseif ($sql_pkg_Pchk_arr['s_no'] * 1 < 100) {
                        $jpgfile = PCS_PKG_URL . '/' . $sql_pkg_Pchk_arr['pkg_no'] . '/' . $sql_pkg_Pchk_arr['pkg_no'] . '_0' . $sql_pkg_Pchk_arr['s_no'];
                    } else {
                        $jpgfile = PCS_PKG_URL . '/' . $sql_pkg_Pchk_arr['pkg_no'] . '/' . $sql_pkg_Pchk_arr['pkg_no'] . '_' . $sql_pkg_Pchk_arr['s_no'];
                    }

                    ?>
                    <tr>
                        <td class="jnt_td"><?php echo $sql_pkg_Pchk_arr['s_no']; ?></td>
                        <td class="jnt_td" style="width:4%"><?php if ($sno_inPKG[$sql_pkg_Pchk_arr['dwg_no']]) {
                                echo $sno_inPKG[$sql_pkg_Pchk_arr['dwg_no']];
                            } else {
                                echo '0';
                            } ?></td>
                        ...
                        <td class="jnt_td" style="padding:0px 0px 0px 15px; text-align:left;">
                            ...
                            if (!$sql_pkg_Pchk_arr['punch_desc'] || $member['mb_7'] == 3) {
                                if ($sql_pkg_Pchk_arr['punch_desc']) {
                                    $punch_desc = $sql_pkg_Pchk_arr['punch_desc'];
                                } else {
                                    $punch_desc = '<p style="text-align:center;font-size:25px;"><font color = red><b> No description </b></font></p>';
                                }
                                ?>
                                <a href='javascript:document.desc_modi<?php echo $sql_pkg_Pchk_arr['s_no']; ?>.submit()'><?php echo $punch_desc; ?></a>
                                <form name='desc_modi<?php echo $sql_pkg_Pchk_arr['s_no']; ?>' method="post"
                                      target="_self" onSubmit="return doSumbit()">
                                    <input type="hidden" name="p_page" value="p_cont">
    ...
    <?php
    $G_total_rt = $bw_s_total + $bw_f_total == 0 ? 'N/A' : ($rt_s_done + $rt_f_done) . ' / ' . ($bw_s_total + $bw_f_total);
    ...
    <table class="main">
        ...
        <tr>
            ...
            <td class="jnt_td">
                <?php if ($member['mb_7'] > 0) {
                    ?>
                    <a href='javascript:document.punch_A.submit()'><?php
                        if (($gen_Apunch_issue) == 0) {
                            echo '-';
                        } else {
                            echo $gen_Apunch_clear . '/' . $gen_Apunch_issue;
                        }
            ...
            <td class="jnt_td">
                <?php if ($member['mb_7'] > 0) {
                    ?>
                    <a href='javascript:document.punch_B.submit()'><?php
                        if (($gen_Bpunch_issue) == 0) {
                            echo '-';
                        } else {
                            echo $gen_Bpunch_clear . '/' . $gen_Bpunch_issue;
                        }
            ...
            <td class="jnt_td">
                <?php if ($member['mb_7'] > 0) {
                    ?>
                    <a href='javascript:document.punch_C.submit()'><?php
                        if (($gen_Cpunch_issue) == 0) {
                            echo '-';
                        } else {
                            echo $gen_Cpunch_clear . '/' . $gen_Cpunch_issue;
                        }
        ...
        while ($sql_pkg_stat_arr = sql_fetch_array($sql_pkg_stat)) {
            ...
            <tr>
                <td class="jnt_td"><a href='javascript:document.submit_for_marked<?php
                    echo $i;
                    ?>.submit()'><font style='font-size:20px;'><?php
                            echo $i;
                            ?> </font></a>
                    <?php
                    viewPDF('submit_for_marked' . $i, 'pkg', $sql_pkg_stat_arr['dwg_no'], $sql_pkg_stat_arr['rev_no'], 'pkg');
                    ?>
                </td>
                <td class="jnt_td"><a href=<?php
                    echo G5_BBS_URL . '/board.php?bo_table=iso&wr_id=' . $sql_pkgby_dwg_ref_arr[wr_id];
                    ?> target='_self'><?php
                ...
                <td class="jnt_td">
                    ...
                    if ($member['mb_7'] > 0) {
                        ?>
                        <a href='javascript:document.punch_<?php
                        echo $i;
                        ?>A.submit()'><?php
                            if (($sql_Apunch_issue) == 0) {
                                echo '-';
                            } else {
                                echo $sql_Apunch_clear . '/' . $sql_Apunch_issue;
                            }
                            ?> </a>
                        <form name='punch_<?php
                        echo $i;
                        ?>A' method="post" target="_self" onSubmit="return doSumbit()">
                            <input type="hidden" name="p_page" value="p_cont">
                            <input type="hidden" name="mode" value="issue">
                            <input type="hidden" name="t_no" value="<?php
                            echo $sql_p_count;
                    ...
                    if ($member['mb_7'] > 0) {
                        ...
                        ?>B.submit()'><?php
                            if (($sql_Bpunch_issue) == 0) {
                                ...
                                echo $sql_Bpunch_clear . '/' . $sql_Bpunch_issue;
                            }
                            ?> </a>
                        <form name='punch_<?php
                        echo $i;
                        ?>B' method="post" target="_self" onSubmit="return doSumbit()">
                            <input type="hidden" name="p_page" value="p_cont">
                            <input type="hidden" name="mode" value="issue">
                            <input type="hidden" name="t_no" value="<?php
                            echo $sql_p_count;
                    ...
                    if ($member['mb_7'] > 0) {
                        ...
                        ?>C.submit()'><?php
                            if (($sql_Cpunch_issue) == 0) {
                                ...
                                echo $sql_Cpunch_clear . '/' . $sql_Cpunch_issue;
                            }
                            ?> </a>
                        <form name='punch_<?php
                        echo $i;
                        ?>C' method="post" target="_self" onSubmit="return doSumbit()">
                            <input type="hidden" name="p_page" value="p_cont">
                            <input type="hidden" name="mode" value="issue">
                            <input type="hidden" name="t_no" value="<?php
                            echo $sql_p_count;
