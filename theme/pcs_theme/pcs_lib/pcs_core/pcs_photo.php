...
if (!empty($_POST['ph'])) {
    ...
    if ($_POST['fn_itm'] == 'tp') {
        ...
        } elseif ($_POST['fn_pho'] == $_POST['fn_jnt']) {
    ...
    $ran = mt_rand(1, 10000);
    if ($_POST['folder'] == 'daily') {
        $thumbIMG = PCS_URL_DAILY . '/' . $_POST['sbjt'] . '/' . $_POST['sbjt'] . '_' . $_POST['j_no'] . '.jpg?ran=' . $ran;
    } elseif ($_POST['photo']) {
        $thumbIMG = PCS_ISO_URL . '/' . $_POST['spdwg'] . '/' . $_POST['photo'] . '.jpg?ran=' . $ran;
    } else {
        $thumbIMG = '';
    }
    ?>
    <div style='background-color:white; padding:10px;'>
        <form method="post" enctype="multipart/form-data" target="_self">
            <input type="hidden" name="fn_sbj" value="<?php
            echo $_POST['sbjt']; ?>">
            <input type="hidden" name="fn_jnt" value="<?php
            echo $_POST['j_no']; ?>">
            <input type="hidden" name="fn_itm" value="<?php
            echo $_POST['folder']; ?>">
            <input type="hidden" name="fn_dwg" value="<?php
            echo $_POST['spdwg']; ?>">
            <input type="hidden" name="fn_pho" value="<?php
            echo $_POST['photo']; ?>">
            ...
            if (!G5_IS_MOBILE) {
                ...
                <tbody>
                <tr>
                    <td class="main_td"><input type="file" name="myFile" id='pcs_thumb' accept='image/*'
                                               style='width:100%'/></td>
                </tr>
                <tr>
                    <td class="main_td"><img id='pcs_prev' width='300px' src="<?php
                        echo $thumbIMG; ?>" alt='thumbnail image'></td>
                ...
                <tr>
                    <td class="main_td">
                        ...
                        <a href="<?php
                        echo G5_BBS_URL ?>/board.php?bo_table=<?php
                        echo $bo_table . '&wr_id=' . $wr_id; ?>" class="btn_cancel">Cancle</a></td>
                </tr>
                </tbody>
            </table>
        </form>
        ...
        pcs_file.onchange = function () {
            ...
            file_content.onload = function () {
                ...
                tempImage.onload = function () {
                    ...
                    canvas.width = <?php if ($_POST['folder'] == 'tp') {
                        echo '1000';
                    } else {
                        echo '800';
                    } ?>;
                    ...
                    var picX = <?php if ($_POST['folder'] == 'tp') {
                        echo '840';
                    } else {
                        echo '640';
                    } ?>;
