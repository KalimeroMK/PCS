...
class TCPDF_COLORS
{
    ...
    public static function convertHTMLColorToDec($hcolor, &$spotc, $defcol = array('R' => 128, 'G' => 128, 'B' => 128))
    {
        ...
        } elseif (substr($color, 0, 4) !== 'cmyk' && substr($color, 0, 3) !== 'rgb' && $dotpos = strpos($color, '.') !== false) {
     ...
     * @public static
     */
    public static function getColorStringFromArray($c): string
    {
        $c = array_values($c);
        $color = '[';
        switch (count($c)) {
     ...
     * @public static
     */
    public static function _JScolor($color): string
    {
        if (substr($color, 0, 1) === '#') {
