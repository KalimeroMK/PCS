...
for ($i = 0; $mysql_field_array[$i]; $i++) {
    ...
    switch ($i) {
        ...
        case 4 :
            if ($_POST[$mysql_field_array[$i]]) {
                $query_insp_spl .= ' AND ' . $mysql_field_array[$i] . ' LIKE "%' . $_POST[$mysql_field_array[$i]] . '%"';
            }
            break;
        ...
        default:
            if ($_POST[$mysql_field_array[$i]]) {
                $query_insp_spl .= ' AND ' . $mysql_field_array[$i] . ' = "' . $_POST[$mysql_field_array[$i]] . '" ';
            }
            break;
    }
}
    ...
    <form name='submit_form' method="post" onSubmit="return doSumbit()">
        <table class="main">
            ...
            while ($sql_insp_spl_arr = sql_fetch_array($sql_insp_spl)) {
                ...
                <input type="hidden" name="lat[<?php echo $no; ?>]"
                       value="<?php echo $sql_insp_spl_arr['gps_lat']; ?>"/>
                <input type="hidden" name="lon[<?php echo $no; ?>]"
                       value="<?php echo $sql_insp_spl_arr['gps_lon']; ?>"/>
                <tr>
                    <td class="jnt_td"><?php echo $no; ?></td>
                    <td class="jnt_td"><a
                                href=<?php echo G5_URL . '/bbs/board.php?bo_table=spool&wr_id=' . $sql_g5_spl_arr['wr_id']; ?> target='_self'>
                            <font style='font-size:25px;'>
                                <b> <?php echo $sql_insp_spl_arr['spool_no']; ?></b></font></a></td>
                    ...
                    <td class="jnt_td"><?php if ($sql_insp_spl_arr['chk_by']) {
                            echo $sql_insp_spl_arr['chk_by'] . '<br>' . $sql_insp_spl_arr['chk_tm'];
                        } else {
                            echo '<b>NONE</b>';
                        } ?></td>
                    <td class="jnt_td"><?php if ($sql_insp_spl_arr['photo_by']) {
                            echo $sql_insp_spl_arr['photo_by'] . '<br>' . $sql_insp_spl_arr['photo_tm'];
                        } ?></td>
                    <td class="jnt_td"><?php if ($sql_insp_spl_arr['photo_by']) {
                            photo_thumb('spool', $sql_insp_spl_arr['photo'], '', 180, $sql_insp_spl_arr['dwg1']);
                        } ?></td>
                </tr>
