...
if (!empty($_POST['dwg_select']) && $_POST['dwg_select'] == 'yes') {
    ...
    <table class="main">
        ...
        <tr>
            ...
            $counter = count($table_field_array);<?php for ($i = 0; $i < $counter; $i++) {
                echo '<td class="jnt_td jnt_th">' . $table_field_array[$i] . '</td>';
            } ?>
        </tr>

        <?php
        $query_dwg = 'SELECT A.* FROM ' . G5_TABLE_PREFIX . 'pcs_info_iso AS A';
        if ($_POST['dwg_stat'] != 'ACT') {
            $query_dwg .= ' LEFT';
        }
        $query_dwg .= ' JOIN ' . G5_TABLE_PREFIX . 'pcs_info_iso_coor AS B ON A.dwg_no  = B.dwg_no';
        if ($_POST['dwg_a_u']) {
            $query_dwg .= ' WHERE ag_ug = "' . $_POST['dwg_a_u'] . '"';
        }
        if ($_POST['dwg_unit']) {
            $query_dwg .= ' AND unit = "' . $_POST['dwg_unit'] . '"';
        }
        ...
        while ($sql_dwg_arr = sql_fetch_array($sql_dwg)) {
            ...
            <tr>
                ...
                <td class="jnt_td"><a
                            href=<?php echo G5_URL . '/bbs/board.php?bo_table=drawing&wr_id=' . $sql_g5_dwg_arr['wr_id']; ?> target='_self'>
                        <font style='font-size:25px;'> <b><?php echo $sql_dwg_arr['dwg_no']; ?></b></font></a></td>
                <td class="jnt_td">
                    <font style='font-size:25px;'> <b> <a
                                    href='javascript:document.submit_for_marked<?php echo $no; ?>.submit()'><b><?php echo $sql_dwg_arr['rev_no']; ?></b>
                    </font> </a>
                    <?php viewPDF('submit_for_marked' . $no, 'fab', $sql_dwg_arr['dwg_no'], $sql_dwg_arr['rev_no']); ?>
                </td>


                $counter = count($sql_field_array);<?php for ($i = 1; $i < $counter; $i++) {
                    echo '<td class="jnt_td">' . $sql_dwg_arr[$sql_field_array[$i]] . '</td>';
                } ?>
            </tr>
    ...
    <table class="main">
        ...
        <tr>
            <td class="main_td" style="width: 80px;"
                rowspan="<?php echo count($j_unit) + 1; ?>"><?php echo $j_agug[$k]; ?></td>
        </tr>
        ...
        <tr>
            <td class="main_td"
            ="width:120px;"><?php echo $j_unit[$j]; ?></td>
            ...
            ?>
            <td class="jnt_td">
                <div class="stat_left"><?php if ($sql_dwg_qty == 0) {
                        echo '';
                    } else {
                        echo $sql_dwg_qty . '<sub> Dwgs.';
                    } ?></div>
                <div class="stat_center"><?php if ($sql_total_SDI == 0) {
                        echo '-';
                    } else {
                        echo sprintf("%.0f", $sql_done_SDI) . ' / ' . sprintf("%.0f", $sql_total_SDI);
                    } ?></div>
                <div class="stat_center"><?php if ($sql_total_FDI != 0) {
                        echo sprintf("%.0f", $sql_done_FDI) . ' / ' . sprintf("%.0f", $sql_total_FDI);
                    } ?></div>
                <div class="stat_right"><?php if (($sql_total_SDI + $sql_total_FDI) == 0) {
                        echo '';
                    } else {
                        echo '(' . sprintf("%.1f", ($sql_done_SDI + $sql_done_FDI) / ($sql_total_SDI + $sql_total_FDI) * 100) . '%)';
                    } ?></div>
            </td>
    ...
    <table class="main" style="width:100%">
        ...
        <tr>
            ...
            <td class="main_td"><?php if ($sql_dwg_total == 0) {
                    echo '-';
                } else {
                    echo $sql_dwg_total;
                } ?></td>
            ...
            for ($i = 0; $i < $j_qty; $i++) {
                if ($i < 2) {
                    dwg_chk_stat($j_agug[$k], $j_unit[$j], $j_state[$i], 'ACT');
                } else {
                    dwg_chk_stat($j_agug[$k], $j_unit[$j], $o_state[$i - 1]);
                }
            }
            } ?>
        ...
        <tr>
            ...
            <td class="main_td td_S_total"><?php if ($sql_dwg_total == 0) {
                    echo '-';
                } else {
                    echo $sql_dwg_total;
                } ?></td>
            ...
            <?php for ($i = 0; $i < $j_qty; $i++) {
                if ($i < 2) {
                    $query_dwg_qty = 'SELECT COUNT(*) FROM ' . G5_TABLE_PREFIX . 'pcs_info_iso AS A JOIN ' . G5_TABLE_PREFIX . 'pcs_info_iso_coor AS B ON A.dwg_no  = B.dwg_no AND B.dwg_state = "' . $j_state[$i] . '" WHERE';
                } else {
                    $query_dwg_qty = 'SELECT COUNT(*) FROM ' . G5_TABLE_PREFIX . 'pcs_info_iso WHERE state = "' . $o_state[$i - 1] . '" AND';
                }
                ...
                <td class="main_td td_S_total"><?php if ($sql_dwg_qty == 0) {
                        echo '-';
                    } else {
                        echo $sql_dwg_qty;
                    } ?></td>
...
{
    if ($st == 'ACT') {
        $query_dwg_qty = 'SELECT COUNT(*) FROM ' . G5_TABLE_PREFIX . 'pcs_info_iso AS A JOIN ' . G5_TABLE_PREFIX . 'pcs_info_iso_coor AS B ON A.dwg_no  = B.dwg_no AND B.dwg_state = "' . $num . '" WHERE';
        $dwg_st = $st;
    } else {
        $query_dwg_qty = 'SELECT COUNT(*) FROM ' . G5_TABLE_PREFIX . 'pcs_info_iso WHERE state = "' . $num . '" AND';
        $dwg_st = $num;
    }

    if ($au !== '' && $au !== '0') {
        $query_dwg_qty .= ' ag_ug = "' . $au . '"';
    }
    if ($un !== '' && $un !== '0') {
        $query_dwg_qty .= ' AND unit = "' . $un . '"';
    }
