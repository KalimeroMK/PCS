		...
		if($_POST['PM_txt']=='clear') {
			...
			if($pkg_coor_info){
				$query_pkg_coor = 'UPDATE '.G5_TABLE_PREFIX.'pcs_info_pkg_coor SET
									...
									tb_qty = "'.$_POST['TB_qty'].'" 
								WHERE dwg_no = "'.$_POST['mapped_dwg'].'" AND pkg_no = "'.$_POST['mapped_pkg'].'" AND rev_no = "'.$_POST['rev_dwg'].'"'; sql_query ($query_pkg_coor);
	...
	<div style='max-width:350px; position: absolute; top: 10px; right: 10px ; height:98%;' >
		<div style='width:100%; height:30%; border:1px solid red;font-size:25px;'>
			...
			<form method="post"> 
				...
				<p align="center">	<input type="submit" id="n_finish" name="n_finish" value="Mapping finish" style="height:35px; width:150px; font-size:18px;" >
			...
			<input type='text' id='pg_no' value='No.' style='height:35px; width:60px; font-size:25px; padding:5px;' onclick="clear_txt('pg_no')"> &nbsp;
			<input type='radio'	name='mark_type' style="height:25px; width: 25px;" value='mark'>Mark 
			<input type='text' id='cmnt' value='cmt.' style='height:35px; width:60px; font-size:25px; padding:5px;' onclick="clear_txt('cmnt')"><br>
			...
			<table id='jnt_table' style="font-size:15px; border-width: 1px 1px 0px 0px; border-style: solid solid none none;" width='330px' border="0" cellspacing="0" cellpadding="0">
				...
				<tbody>
					<tr style="height: 30px; background-color: aquamarine;">
						<td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 50px" ><p align="center">no.</p></td>
						<td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 50px" ><p align="center">seq.</p></td>
						<td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 50px" ><p align="center">Mark<br></p></td>
						<td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 50px" ><p align="center">TB no</p></td>
						<td style="border-width: 0px 0px 1px 1px; border-style: none none solid solid; width: 80px" ><p align="center">content</p></td>
					</tr>
...
function dwgrefresh(rec_Pinfo) {
	...
	for (var i=0; i<PMArray.length-1; i++) {
		...
		if(tab_tf){Sno = i;}
			...
			switch (PMData[5]) {
				case 'line' :
					if(PMData[8]=='Act'){drawLine(PMData[1],PMData[2],PMData[3],PMData[4]);}
					break;
				case 'page' :
					if(PMData[8]=='Act'){drawPage(PMData[1]*1,PMData[2]*1,PMData[3]*1,PMData[4]*1,PMData[7]);}
					if(tab_tf){addRowInTable(PMData[8],TBqty,PMData[6],PMData[5],PMData[7]);}
					break;
				case 'mark' :
					if(PMData[8]=='Act'){drawPage(PMData[1]*1,PMData[2]*1,PMData[3]*1,PMData[4]*1,PMData[7]);}
					if(tab_tf){addRowInTable(PMData[8],TBqty,PMData[6],PMData[5],PMData[7]);}
					break;
				case 'blind':
					if(PMData[8]=='Act'){drawBlind(PMData[1],PMData[2],PMData[3],PMData[4]);}
					if(tab_tf){addRowInTable(PMData[8],TBqty,PMData[6],PMData[5],PMData[7]);}
					break;
				case 'tbno' :
					if(PMData[8]=='Act'){drawTB(PMData[1]*1,PMData[2]*1,PMData[3]*1,PMData[4]*1,PMData[7]*1 + TBsum);
					TBqty = PMData[7]*1;}
					if(tab_tf){addRowInTable(PMData[8],TBqty,PMData[6],PMData[5],PMData[7]);}
					break;
				default :	break;
...
function drawMark() {
	...
	context.lineWidth = 3;
	context.beginPath();
	var wtx = 250;	var hty = 40;
	var tx = 1940;	var ty = 1000;
...
function addRowInTable(jntState,tbnum,tbsize,typ,pc) {
	...
	newTr = jntTable.insertRow(rowIndex);
		newTr.align = 'center';
		if(jntState == 'Del'){newTr.style = 'background-color: red';}
	...
	newTd3 = newTr.insertCell(2);
		...
		if(typ=='page'||typ=='mark'){newTd3.innerHTML = typ;intxt = pc;}
		else {newTd3.innerHTML = typ;intxt = tbsize}
		$(newTd3).on("click", newTd3, function(){trDelete(rowIndex-1,sarr[rowIndex]);});
	...
	newTd4 = newTr.insertCell(3);
		...
		switch(typ){
			case 'blind':	newTd4.innerHTML = '-';	break;
			case 'tbno':	newTd4.innerHTML = tbnum+TBsum;	break;
			case 'page':	newTd4.innerHTML = '-';	break;
			default : break;
		}
	...
	newTd5 = newTr.insertCell(4);
		...
		if(typ=='tbno'||typ=='page'||typ=='mark'){
			...
			if(!txb3.value) {$(txb3).css('background-color','red');};
			
			newTd5.appendChild(txb3);
			
			$(txb3).focusin(function(){
				bftxb = txb3.value; txb3.value = ''; $(txb3).css('background-color','white');
			});
			
			$(txb3).focusout(function(){
				if(!txb3.value) {txb3.value = bftxb;};
