...
if ($_POST['folder'] || $_POST['ph']) {
    ...
    <form name="desc" method="post" onSubmit="return doSumbit()">
        ...
        <table class="main">
            <caption>
                <p style="text-align:center; font-size:50px;"><a href="javascript:document.desc.submit()">REPORT
                        Description Edit</a></p>
            ...
            <tr>
                <td class="jnt_td"><?php
                    echo $desc_no;
                    ?></td>
                <td class="jnt_td"><input type="text" name="desc_after" autocomplete="off"
                                          style="padding:0px 0px 0px 15px; text-align:left;width:98%;height:50px;font-size:20px;border:none;"
                                          value="<?php
                                          echo $daily_cont[$desc_no];
                                          ?>"></td>
                <td class="jnt_td">
    ...
    */
    ...
    $issue_idx = $sql_daily_arr['wr_1'];
    if ($view['wr_subject'] == G5_TIME_YMD && $member['mb_2'] > 1) {
        $issue_idx++;
    }
                    ...
                    <td class="jnt_td">
                        ...
                        if ($view['wr_subject'] == G5_TIME_YMD && $member['mb_2'] > 1 && $i >= $sql_daily_arr['wr_1'] - 1) {
                            if ($daily_isur[$i + 1] !== '' && $daily_isur[$i + 1] !== '0') {
                                if ($member['mb_nick'] == $daily_isur[$i + 1]) {
                                    photo_up('daily', $view['wr_subject'], $i, $sql_daily_arr['wr_1'], $sql_daily_arr['wr_2']);
                                }
                            } else {
                                photo_up('daily', $view['wr_subject'], $i, $sql_daily_arr['wr_1'], $sql_daily_arr['wr_2']);
                            }
