...
$bf_content = isset($_POST['bf_content']) ? (array)$_POST['bf_content'] : [];
...
if (!$is_admin && !$board['bo_use_secret'] && (stripos($_POST['html'], 'secret') !== false || stripos($_POST['secret'],
            'secret') !== false || stripos($_POST['mail'], 'secret') !== false)) {
    alert('비밀글 미사용 게시판 이므로 비밀글로 등록할 수 없습니다.');
}

$secret = '';
if (isset($_POST['secret']) && $_POST['secret'] && preg_match('#secret#', strtolower($_POST['secret']), $matches)) {
    $secret = $matches[0];
}
...
$html = '';
if (isset($_POST['html']) && $_POST['html'] && preg_match('#html(1|2)#', strtolower($_POST['html']), $matches)) {
    $html = $matches[0];
}

$mail = '';
if (isset($_POST['mail']) && $_POST['mail'] && preg_match('#mail#', strtolower($_POST['mail']), $matches)) {
    $mail = $matches[0];
}
...
for ($i = 1; $i <= 10; $i++) {
    $var = "wr_$i";
    $$var = "";
    if (isset($_POST['wr_' . $i]) && (string)$_POST['wr_' . $i]) {
...
if ($w == '' || $w == 'u') {
    ...
    if (!($w == 'u' && $member['mb_id'] && $wr['mb_id'] === $member['mb_id']) && $member['mb_level'] < $board['bo_write_level']) {
        alert('글을 쓸 권한이 없습니다.');
    }
} elseif ($w == 'r') {
...
if ($w == '' || $w == 'r') {
    if (isset($_SESSION['ss_datetime']) && ($_SESSION['ss_datetime'] >= G5_SERVER_TIME - $config['cf_delay_sec'] && !$is_admin)) {
...
if ($w == '' || $w == 'r') {
    ...
    if ($w == '') {
        ...
        insert_point($member['mb_id'], $board['bo_write_point'], "{$board['bo_subject']} {$wr_id} 글쓰기", $bo_table,
            $wr_id, '쓰기');
    } else {
        // 답변은 코멘트 포인트를 부여함
        // 답변 포인트가 많은 경우 코멘트 대신 답변을 하는 경우가 많음
        insert_point($member['mb_id'], $board['bo_comment_point'], "{$board['bo_subject']} {$wr_id} 글답변", $bo_table,
            $wr_id, '쓰기');
    }
} elseif ($w == 'u') {
    ...
    if ($member['mb_id']) {
        ...
        $mb_id = "";
        // 비회원의 경우 이름이 누락되는 경우가 있음
        if (trim($wr_name) === '' || trim($wr_name) === '0') {
            alert("이름은 필히 입력하셔야 합니다.");
        }
        $wr_name = clean_xss_tags(trim($_POST['wr_name']));
        $wr_email = get_email_address(trim($_POST['wr_email']));
...
} elseif ($file_count > $board['bo_upload_count']) {
...
if (isset($_FILES['bf_file']['name']) && is_array($_FILES['bf_file']['name'])) {
    $counter = count($_FILES['bf_file']['name']);
    for ($i = 0; $i < $counter; $i++) {
        $upload[$i]['file'] = '';
        $upload[$i]['source'] = '';
        $upload[$i]['filesize'] = 0;
        ...
        if (isset($_POST['bf_file_del'][$i]) && $_POST['bf_file_del'][$i]) {
            ...
            $delete_file = run_replace('delete_file_path',
                G5_DATA_PATH . '/file/' . $bo_table . '/' . str_replace('../', '', $row['bf_file']), $row);
...
if ($w != 'u' && $w != 'cu' && $config['cf_email_use'] && $board['bo_use_email']) {
    ...
    if (isset($wr['wr_option']) && isset($wr['wr_email']) && (strstr($wr['wr_option'], 'mail') && $wr['wr_email'])) {
        $array_email[] = $wr['wr_email'];
    }
...
$redirect_url = run_replace('write_update_move_url',
    short_url_clean(G5_HTTP_BBS_URL . '/board.php?bo_table=' . $bo_table . '&amp;wr_id=' . $wr_id . $qstr), $board, $wr_id, $w,
    $qstr, $file_upload_msg);
