<?php
// Fix: prevent undefined array key warnings
if (isset($_POST['spl_select']) && $_POST['spl_select'] == 'yes'){
...
<table class="main">
    ...
    <tbody>
    <tr>
        $counter = count($table_field_array);<?php
        for ($i = 0; $i < $counter; $i++) {
            echo '<td class="jnt_td jnt_th" style="width: ' . $table_width_array[$i] . 'px">' . $table_field_array[$i] . '</td>';
        } ?>
    </tr>
    ...
    if (isset($_POST['spl_a_u']) && $_POST['spl_a_u']) {
        $query_spool .= ' WHERE ag_ug = "' . $_POST['spl_a_u'] . '"';
    }
    if (isset($_POST['spl_unit']) && $_POST['spl_unit']) {
        $query_spool .= ' AND unit = "' . $_POST['spl_unit'] . '"';
    }
    if ((isset($_POST['spl_stat']) && $_POST['spl_stat']) || (isset($_POST['spl_loca']) && $_POST['spl_loca'])) {
        if (isset($_POST['spl_stat']) && $_POST['spl_stat']) {
            $query_spool .= ' AND state = "' . $_POST['spl_stat'] . '"';
        } else {
            $query_spool .= ' AND state = "Finished"';
        }
        if (isset($_POST['spl_loca']) && $_POST['spl_loca']) {
            $query_spool .= ' AND location = "' . $_POST['spl_loca'] . '"';
        }
    }
    ...
    while ($sql_spool_arr = sql_fetch_array($sql_spool)) {
        ...
        <tr>
            <td class="jnt_td">
                <a href='javascript:document.smt_<?php echo $no; ?>.submit()'><font style='font-size:25px;'>
                        <b> <?php echo $no; ?> </b> </font></a>
                <?php viewPDF('smt_' . $no, 'fab', $ref_dwg, $sql_g5_dwg_arr['rev_no'] ?? ''); ?>
            </td>
            <td class="jnt_td">
                <a href=<?php echo G5_URL . '/bbs/board.php?bo_table=spool&wr_id=' . $sql_g5_spool_arr['wr_id']; ?> target='_self'>
                    <font style='font-size:25px;'> <b> <?php echo $sql_spool_arr['spool_no']; ?></b></font></a>
            </td>

            $counter = count($sql_field_array);<?php for ($i = 0; $i < $counter; $i++) {
                echo '<td class="jnt_td">' . str_replace('_', ' ', $sql_spool_arr[$sql_field_array[$i]] ?? '') . '</td>';
            } ?>
...
<p>&nbsp;
    ...
    $row_agug = $field_val['ag_ug'] ?? array();
    $row_unit = $field_val['unit'] ?? array();

    $field_query = 'DESCRIBE ' . G5_TABLE_PREFIX . 'pcs_info_spl_stat';
    $field_val = enum_value($field_query);

    $col_stat = $field_val['state'] ?? array();
    $col_loca = $field_val['location'] ?? array();
    ?>
...
<table class="main">
    ...
    <tr>
        <?php
        for ($i = 0; isset($col_stat[$i]) && $col_stat[$i]; $i++) {
            echo '<td class="main_td jnt_th" style="font-size:20px;">' . str_replace('_', ' ', $col_stat[$i]) . '</td>';
        }
        for ($i = 0; isset($col_loca[$i]) && $col_loca[$i]; $i++) {
            echo '<td class="main_td jnt_th" style="font-size:20px;">' . $col_loca[$i] . '</td>';
        }
        ?>
    </tr>
    ...
    <tr>
        ...
        for ($i = 0; isset($col_stat[$i]) && $col_stat[$i]; $i++) {
            spl_fab_stat($row_agug[$k], $row_unit[$j], $col_stat[$i], '');
        }

        for ($i = 0; isset($col_loca[$i]) && $col_loca[$i]; $i++) {
            spl_loc_stat($row_agug[$k], $row_unit[$j], '', $col_loca[$i]);
        }
        }
        ?>
    </tr>
    <tr>
        ...
        for ($i = 0; isset($col_stat[$i]) && $col_stat[$i]; $i++) {
            spl_fab_stat($row_agug[$k], '', $col_stat[$i], '', 'td_S_total');
        }
        for ($i = 0; isset($col_loca[$i]) && $col_loca[$i]; $i++) {
            spl_loc_stat($row_agug[$k], '', '', $col_loca[$i], 'td_S_total');
        }
        }
        ?>
    </tr>
    <tr>
        ...
        for ($i = 0; isset($col_stat[$i]) && $col_stat[$i]; $i++) {
            spl_fab_stat('', '', $col_stat[$i], '', 'td_G_total');
        }
        for ($i = 0; isset($col_loca[$i]) && $col_loca[$i]; $i++) {
            spl_loc_stat('', '', '', $col_loca[$i], 'td_G_total');
        }
        ?>
...
<p>&nbsp;
    ...
    function spl_fab_stat(string $au, string $un, string $st, string $lo, string $co = ''): void
    {
        ...
        if ($au !== '' && $au !== '0') {
            $query_spl_qty .= ' WHERE ag_ug = "' . $au . '"';
        }
        if ($un !== '' && $un !== '0') {
            $query_spl_qty .= ' AND unit = "' . $un . '"';
        }
        if ($st || $lo) {
            if ($st !== '' && $st !== '0') {
                $query_spl_qty .= ' AND state = "' . $st . '"';
            } else {
                $query_spl_qty .= ' AND state = "Finished"';
            }
            if ($lo !== '' && $lo !== '0') {
                $query_spl_qty .= ' AND location = "' . $lo . '"';
            }
        }
