...
if($_POST['folder'] || $_POST['ph']){include_once (PCS_LIB.'/pcs_photo.php');}
	...
	else {$query_spl_stat .= '"N/A"';}
	$query_spl_stat .= ', st_pmi = ';

	if($pmi_total){$query_spl_stat .= '"'.$pmi_done.' / '.$pmi_total.'"';}
	else {$query_spl_stat .= '"N/A"';}
...
<?php
	for ($j=1;$j<4;$j++){
		...
		else {echo '<td class="main_td" style="width:70px; font-size:15px;"></td><td class="main_td"></td>';}
	}
?>
</tr>
</tbody>
...
<td class="main_td" style="height:80px;"> <?php echo $sql_ref_dwg_info_arr['pmi'].' / '.$sql_ref_dwg_info_arr['pwht']; ?>  </td>
...
<?php
	...
	else {echo $sql_spl_info_arr['location'];}
?>
</td>
...
<?php 
	...
	else {echo str_replace('_',' ',$sql_spl_info_arr['state']);}
?>
</td>
...
<td class="jnt_td" rowspan="2" ><?php echo $PCS_DEL;	echo $sql_ref_sbc_arr['dwg_no']; ?></td>
...
<?php
	echo $PCS_DEL;	
	if($sql_ref_sbc_arr['j_stat'] == 'DEL'){ echo $sql_ref_sbc_arr['pcs_fitup_req_date'].'<br>'.$sql_ref_sbc_arr['pcs_fitup_rlt'];} 
...
<tbody>
<tr>
<td class="main_td td_sub" style='background-color: gold;' colspan="2" > <?php echo $view['wr_subject'];?></td>
</tr>
<tr>
<td class="main_td td_sub"> MATERIAL </td>
<td class="main_td"> <?php echo $sql_ref_dwg_info_arr['material']; ?> </td>
...
<tr>
	<td class='jnt_td jnt_th' style="width: 25%;">	Joint no.<br><font size="5"><?php echo z_rem_jno($sql_ref_sbc_arr['j_no']); ?></font></td>
