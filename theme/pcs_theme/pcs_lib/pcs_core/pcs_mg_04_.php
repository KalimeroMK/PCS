    ...
    <tr>
        ...
        <table class="main">
            ...
            while ($sql_daily_list_arr = sql_fetch_array($sql_daily_list)) {
                ...
                ?>
                <tr>
                    <td class="jnt_td" style="width: 50px"><a
                                href='javascript:document.submit_for<?php echo $jnt_qty; ?>.submit()'> <font
                                    style='font-size:25px;'> <b> <?php echo $jnt_qty; ?> </b></font></a></td>
                    <form name='submit_for<?php echo $jnt_qty; ?>' action="<?php echo PCS_WPV_URL; ?>/viewer.php"
                          method="post" target="<?php echo $sql_daily_list_arr['dwg_no']; ?>"
                    ...
                    <td class="jnt_td" style="width:300px"><a
                                href=<?php echo G5_URL . '/app/board/board.php?bo_table=drawing&wr_id=' . $sql_g5_dwg_arr[wr_id]; ?> target='_self'>
                            <font style='font-size:25px;'> <b> <?php echo $sql_daily_list_arr['dwg_no']; ?> </b></font></a>
                    </td>
                    <td class="jnt_td"
                        style="width:100px"> <?php preg_match('/[A-Z1-9][A-Z0-9]*/', $sql_daily_list_arr['j_no'], $mts);
                        echo $mts[0]; ?> </td>
                    ...
                    <td class="jnt_td" style="width:200px"> <?php if ($sql_sql_pcs_list_arr['item_1_photo']) {
                            echo "<a onclick=\"window.open('" . PCS_THUMB_URL . '/' . $sql_sql_pcs_list_arr['item_1_photo'] . ".jpg','w','width=600');\">" . '<img src="' . PCS_THUMB_URL . '/thumb-' . $sql_sql_pcs_list_arr['item_1_photo'] . '_120x90.jpg"><br>';
                        } ?> </td>
                    <td class="jnt_td" style="width:200px"> <?php if ($sql_sql_pcs_list_arr['item_2_photo']) {
                            echo "<a onclick=\"window.open('" . PCS_THUMB_URL . '/' . $sql_sql_pcs_list_arr['item_2_photo'] . ".jpg','w','width=600');\">" . '<img src="' . PCS_THUMB_URL . '/thumb-' . $sql_sql_pcs_list_arr['item_2_photo'] . '_120x90.jpg"><br>';
                        } ?> </td>
                </tr>
            ...
            for ($i = 0; $i < 8; $i++) {
                $wdate[$i] = substr(date('Y-m-d H:i:s', time() - (3600 * 6) - (3600 * 24 * (7 - $i))), 0, 10);
            }
            ?>
            <table class="main">
                ...
                <tr>
                    <?php for ($i = 0; $i < 7; $i++) {
                        echo '<td class="jnt_td jnt_th" >' . $wdate[$i] . '</td>';
                    } ?>
                ...
                while ($sql_welder_list_arr = sql_fetch_array($sql_welder_list)) {
                    ...
                    <tr>
                        ...
                        for ($i = 0; $i < 7; $i++) {
                            ...
                            if (pcs_sql_value($query_welder_di)) {
                                $weak_acc_day++;
                            }
                        ...
                        if (pcs_sql_value($query_welder_day)) {
                ...
                <tr>
                    ...
                    <?php
                    for ($i = 0; $i < 7; $i++) {
                        echo '<td class="jnt_td td_S_total" ><a href=' . G5_URL . '/app/board/board.php?bo_table=status&wr_id=3&wdr_select=daily_by&wdate=' . $wdate[$i - 1] . ' target="_self"><font style="font-size:25px;"> <b>' . $G_total_val[$i] . '</b></font></a></td>';
                    }
