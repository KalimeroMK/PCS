...
function get_pretty_url(string $folder, ?string $no = '', ?string $query_string = '', $action = '')
{
    global $g5, $config;
...
function short_url_clean($string_url, ?string $add_qry = '')
{
...
function correct_goto_url(string $url): string
{
...
function generate_seo_title($string, $wordLimit = G5_SEO_TITLE_WORD_CUT): string
{
    $separator = '-';
...
function check_case_exist_title($data, $case = G5_BBS_DIR, $is_redirect = false): void
{
    ...
    } elseif ($case == G5_CONTENT_DIR && isset($data['co_seo_title'])) {
    ...
    } elseif (defined('G5_SHOP_DIR') && $case == G5_SHOP_DIR && isset($data['it_seo_title'])) {
        $db_table = $g5['g5_shop_item_table'];
        if (shop_exist_check_seo_title($data['it_seo_title'], $case, $db_table, $data['it_id']) !== '' && shop_exist_check_seo_title($data['it_seo_title'], $case, $db_table, $data['it_id']) !== '0') {
...
function seo_title_update($db_table, $pk_id, $type = 'bbs'): void
{
...
function get_nginx_conf_rules($return_string = false): string|array
{
...
function get_mod_rewrite_rules($return_string = false): string|array
{
    $get_path_url = parse_url(G5_URL);
    $base_path = isset($get_path_url['path']) ? $get_path_url['path'] . '/' : '/';
...
function check_need_rewrite_rules(): bool
{
...
function update_rewrite_rules(): bool
{
