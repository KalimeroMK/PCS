<?php 
	...
	else if($_POST['j_mode']){include_once (PCS_LIB.'/pcs_info_jnt_edit.php');}
	else {
...
<tr>
<?php
if($sql_dwg_arr['con_dwg']){ echo '<td class="main_td" colspan=6 style="background-color: #F6D8CE; height:50px;"><b>CONTINUED DRAWING</td></tr>';
...
<?php
		if($con_dwg_array[$i]){
			...
			else {echo '<mark>'.$con_no.'. '.$con_dwg_array[$i].'</mark>';}
...
<tr>
<?php
if($sql_dwg_arr['inc_tp']){ echo '<td class="main_td" colspan=6 style="background-color: #BCF5A9; height:50px;"><b>INCLUDED TP</td></tr>';
...
<?php
		if($inc_tp_array[$i]){
			...
			else {echo '<mark>'.$con_no.'. '.$inc_tp_array[$i].'</mark>';}
		}
?>
...
<?php
	...
	if(strstr($sql_dwg_coor_array['joint_info'],'spool')){ echo '<td class="main_td" colspan=6 style="background-color: #BCF5A9; height:80px;"><b>INCLUDED SPOOL</td></tr>';}
	$j=0;
...
//	echo $jnt_each_arr[5].'-SP0'.$jnt_each_arr[10];
	if($jnt_each_arr[5]==$view['wr_subject']) {echo 'SP-0'.$jnt_each_arr[10];}
	else {echo '<mark><b>&lt;SP-0'.$jnt_each_arr[10].'&gt;<b></mark>';}
?>
</a>
</td>
<?php		
...
<?php
	...
	$query_JOINT = 'SELECT * FROM '.G5_TABLE_PREFIX.'pcs_info_joint WHERE dwg_no = "'.$view['wr_subject'].'" ORDER BY j_no';
	$sql_ref_JOINT = sql_query ($query_JOINT, true);


while ($sql_ref_sbc_arr = sql_fetch_array ($sql_ref_JOINT))	{
...
<td class="td_upper" >	<?php echo $PCS_DEL;	echo $sql_ref_sbc_arr['j_type']; ?></td>
<td class="td_upper" >	<?php echo $PCS_DEL;	echo $sql_ref_sbc_arr['s_f']; ?></td>
<td class="td_upper" >	<?php echo $PCS_DEL;	echo $sql_ref_sbc_arr['j_size']; ?></td>
...
<?php
	...
	elseif($sql_ref_sbc_arr['j_stat'] == 'DEL'){echo $sql_ref_sbc_arr['pcs_vi_rlt_date'].'<br>'.$sql_ref_sbc_arr['pcs_vi_rlt'];} 
	elseif($sql_ref_sbc_arr['pcs_fitup_rlt'] == 'Accept'){
		if($member['mb_4']<2 && $member['mb_nick'] != $sql_ref_sbc_arr['pcs_vi_rlt_by'] && $sql_ref_sbc_arr['pcs_vi_rlt_by'] !='' ) {echo $sql_ref_sbc_arr['pcs_vi_rlt_date'].'<br>'.$sql_ref_sbc_arr['pcs_vi_rlt'];} 
		else {insp_vi($idx, $sql_ref_sbc_arr['dwg_no'], $sql_ref_sbc_arr['j_no'], $member['mb_4'], $member['mb_nick'], $sql_ref_sbc_arr['pcs_vi_req_date'], $sql_ref_sbc_arr['pcs_vi_rlt'], $sql_ref_sbc_arr['pcs_vi_rlt_date'], $sql_ref_sbc_arr['pcs_pwht_req_date'], $sql_ref_sbc_arr['pcs_pmi_req_date'], $sql_ref_sbc_arr['pcs_nde_req_date']);}
	}
	else {echo 'Not yet<br>Fit-up';}
?>
</td>
...
<td class="td_lower"><?php echo $sql_ref_con_arr['j_stat']; ?></tr>
<?php
//	}
...
<tr>
<?php if($sql_dwg_arr['pkg_no1']){echo '<td class="main_td" colspan="2"><a href="'.G5_BBS_URL.'/board.php?bo_table=package&wr_id='.$sql_pkg1_arr['wr_id'].'" target="_self">'.$sql_dwg_arr['pkg_no1'].'</a></td>';} ?> 
</tr>
<tr>
<?php if($sql_dwg_arr['pkg_no2']){echo '<td class="main_td" colspan="2"><a href="'.G5_BBS_URL.'/board.php?bo_table=package&wr_id='.$sql_pkg2_arr['wr_id'].'" target="_self">'.$sql_dwg_arr['pkg_no2'].'</a></td>';} ?> 
</tr>
-->
...
<?php
if($sql_dwg_arr['inc_tp']){ echo '<tr><td class="main_td" colspan=6 style="background-color: #BCF5A9; height:50px;"><b>INCLUDED TP</td></tr>';
...
<tr><td class="jnt_td" style='height:80px;font-size:15px;'>
<?php
		if($inc_tp_array[$i]){
			$j++;
			$con_no = $i+1;
...
<tr>
	<td class='jnt_td jnt_th' style="width: 25%;">Joint no.<br><font size="5">
...
<script language="javascript">
	function jointEdit(dn,con_dwg) {window.open("<?php echo PCS_LIB_URL; ?>/pcs_info_jnt_edit.php?dn="+dn+"&con_dwg="+con_dwg, 'jointedit'+dn, "width=1600, height=800, left=200, top=100");}
	$('html, body').stop().animate({scrollTop : $("#tr<?php echo $_POST['html_loc'];?>").offset().top - screen.height/2},300);
	
	function dom_hide(el) {
		if(document.getElementById(el).hidden){document.getElementById(el).hidden = false;}
		else {document.getElementById(el).hidden = true;}
	}
</script>
