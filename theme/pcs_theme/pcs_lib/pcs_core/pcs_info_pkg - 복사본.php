...
/*			$query_dwg = 'SELECT dwg_no, rev_no FROM '.G5_TABLE_PREFIX.'pcs_info_iso WHERE dwg_no = "'.$sql_pkg_stat_arr['dwg_no'].'"';		// 쿼리문
			...
			else {$pkg_table_array[$i][15][0] = ($sql_Apunch_clear).' / '. ($sql_Apunch_issue); $pkg_table_array[$i][15][1] = $sql_Apunch_clear != $sql_Apunch_issue;}
			if ( ($sql_Bpunch_issue) == 0) {$pkg_table_array[$i][16][0] = '-';$pkg_table_array[$i][16][1] = false;}
			else {$pkg_table_array[$i][16][0] = ($sql_Bpunch_clear).' / '. ($sql_Bpunch_issue); $pkg_table_array[$i][16][1] = $sql_Bpunch_clear != $sql_Bpunch_issue;}
			if ( ($sql_Cpunch_issue) == 0) {$pkg_table_array[$i][17][0] = '-';$pkg_table_array[$i][17][1] = false;}
			else {$pkg_table_array[$i][17][0] = ($sql_Cpunch_clear).' / '. ($sql_Cpunch_issue); $pkg_table_array[$i][17][1] = $sql_Cpunch_clear != $sql_Cpunch_issue;}
...
<td class="jnt_td" style="background-color:
    ...
    echo 'red';
} else {echo 'gold';} ?>; font-size:12px" colspan=2>
...
<?php 
		...
		for($j=0;$j<$tmp_count;$j++){
			if($pkg_page_no[$i][$j] !== 0){echo $pkg_table_array[$i][6][$j].' ';} else {
...
<td class="jnt_td" <?php if($pkg_table_array[$i][8][0] !== 0){echo 'style="background-color:red"';} ?>><?php echo $pkg_table_array[$i][8][1]; ?></td>

<td class="jnt_td" <?php if($pkg_table_array[$i][9][0] !== 0){echo 'style="background-color:red"';} ?>><?php echo $pkg_table_array[$i][9][1]; ?></td>
...
<?php if ($member['mb_7']>0) {
    ...
    if ( ($gen_Apunch_issue) == 0) {echo '-';} else {echo $gen_Apunch_clear.'/'.$gen_Apunch_issue;}
    ?> </a>
...
<?php if ($member['mb_7']>0) {
    ...
    if ( ($gen_Bpunch_issue) == 0) {echo '-';} else {echo $gen_Bpunch_clear.'/'.$gen_Bpunch_issue;}
    ?> </a>
...
<?php if ($member['mb_7']>0) {
	...
	<a href = 'javascript:document.punch_C.submit()'><?php 
     if ( ($gen_Cpunch_issue) == 0) {echo '-';} else {echo $gen_Cpunch_clear.'/'.$gen_Cpunch_issue;}
     ?> </a>
...
<?php 
	...
	<a href = 'javascript:document.punch_<?php 
            echo $i;
            ?>A.submit()'><?php 
            if ( ($sql_Apunch_issue) == 0) {echo '-';} else {echo $sql_Apunch_clear.'/'.$sql_Apunch_issue;}
            ?> </a>
...
<?php 
	...
	<a href = 'javascript:document.punch_<?php 
            echo $i;
            ?>B.submit()'><?php 
            if ( ($sql_Bpunch_issue) == 0) {echo '-';} else {echo $sql_Bpunch_clear.'/'.$sql_Bpunch_issue;}
            ?> </a>
	<form name='punch_<?php 
            echo $i;
...
<?php 
	...
	<a href = 'javascript:document.punch_<?php 
            echo $i;
            ?>C.submit()'><?php 
            if ( ($sql_Cpunch_issue) == 0) {echo '-';} else {echo $sql_Cpunch_clear.'/'.$sql_Cpunch_issue;}
            ?> </a>
	<form name='punch_<?php 
            echo $i;
