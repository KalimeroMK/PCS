...
<td class="main_td">
<input type='text' name='iso_name' style='padding:0px 0px 0px 5px; text-align:left;width:90%;height:30px;font-size:15px;background-color:bisque;'>
</td>
<td class="main_td">
...
<?php
		...
		if($_POST[log_type]=='SPOOL'){
			...
			if($_POST['unit']){$query_jnt .= "AND unit = '".$_POST['unit']."' ";};
			if($_POST[material]){$query_jnt .= "AND material = '".$_POST[material]."' ";};
...
<?php			
			...
			switch($_POST[log_type]){
				case 'WELDING'	:	$query_jnt .= "A.pcs_j_type != 'SPL' AND A.pcs_j_type != 'PS' ";	break;
				case 'SUPPORT'	:	$query_jnt .= "A.pcs_j_type = 'PS' ";		break;
				default :	break;
			}
	
			if($_POST['unit']){$query_jnt .= "AND A.unit = '".$_POST['unit']."' ";};
			if($_POST[material]){$query_jnt .= "AND B.material = '".$_POST[material]."' ";};
			if($_POST[s_f]){$query_jnt .= "AND A.s_f = '".$_POST[s_f]."' ";};
