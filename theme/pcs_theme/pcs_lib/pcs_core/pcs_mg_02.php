...
//    $query_spool =  'SELECT A.*, B.* FROM '.G5_TABLE_PREFIX.'pcs_info_spool AS A LEFT JOIN '.G5_TABLE_PREFIX.'pcs_info_spl_stat AS B ON A.spool_no = B.spool_no';
        ...
        if(isset($_POST['spl_a_u']) && $_POST['spl_a_u']) {$query_spool .= ' WHERE ag_ug = "'.$_POST['spl_a_u'].'"';}
        if(isset($_POST['spl_unit']) && $_POST['spl_unit']) {$query_spool .= ' AND unit = "'.$_POST['spl_unit'].'"';}
        if((isset($_POST['spl_stat']) && $_POST['spl_stat']) || (isset($_POST['spl_loca']) && $_POST['spl_loca'])) {
            if(isset($_POST['spl_stat']) && $_POST['spl_stat']) {$query_spool .= ' AND state = "'.$_POST['spl_stat'].'"';} else {$query_spool .= ' AND state = "Finished"';}
            if(isset($_POST['spl_loca']) && $_POST['spl_loca']) {$query_spool .= ' AND location = "'.$_POST['spl_loca'].'"';}
        }
...
<tr>
<?php
    for ($i=0; isset($col_stat[$i]) && $col_stat[$i]; $i++)    { echo '<td class="main_td jnt_th" style="font-size:20px;">'.str_replace('_',' ',$col_stat[$i]).'</td>';} 
    for ($i=0; isset($col_loca[$i]) && $col_loca[$i]; $i++)    { echo '<td class="main_td jnt_th" style="font-size:20px;">'.$col_loca[$i].'</td>';} 
?>
</tr>

<?php
    for ( $k = 0 ; isset($row_agug[$k]) && $row_agug[$k] ; $k++ ) {
?>
<tr>
<td class="main_td" rowspan="<?php echo count($row_unit)+1; ?>"><?php echo $row_agug[$k]; ?></td>
</tr>
<?php
        for ( $j = 0 ; isset($row_unit[$j]) && $row_unit[$j] ; $j++ )    {
?>    
<tr>
...
<?php
        ...
        for ($i=0; isset($col_stat[$i]) && $col_stat[$i]; $i++)    {spl_fab_stat($row_agug[$k],$row_unit[$j],$col_stat[$i],'');}

        for ($i=0; isset($col_loca[$i]) && $col_loca[$i]; $i++)    {spl_loc_stat($row_agug[$k],$row_unit[$j],'',$col_loca[$i]);}
    }
?>
</tr>
<tr>
...
<?php
    ...
    for ($i=0; isset($col_stat[$i]) && $col_stat[$i]; $i++)    {spl_fab_stat($row_agug[$k],'',$col_stat[$i],'','td_S_total');}
    for ($i=0; isset($col_loca[$i]) && $col_loca[$i]; $i++)    {spl_loc_stat($row_agug[$k],'','',$col_loca[$i],'td_S_total');}
}
?>
</tr>
<tr>
...
for ($i=0; isset($col_stat[$i]) && $col_stat[$i]; $i++)    {spl_fab_stat('','',$col_stat[$i],'','td_G_total');}
for ($i=0; isset($col_loca[$i]) && $col_loca[$i]; $i++)    {spl_loc_stat('','','',$col_loca[$i],'td_G_total');}
?>
...
function spl_fab_stat($au, $un, $st, $lo, $co='') {
    ...
    if($st||$lo) {
        if($st !== '' && $st !== '0') {$query_spl_qty .= ' AND state = "'.$st.'"';} else {$query_spl_qty .= ' AND state = "Finished"';}
        if($lo !== '' && $lo !== '0') {$query_spl_qty .= ' AND location = "'.$lo.'"';}
    }
