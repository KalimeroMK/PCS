                ...
                $counter = count($list2);<?php for ($i = 0; $i < $counter; $i++) { ?>
                    <article>
                        <header>
                            ...
                            <span class="poll_datetime"><i class="fa fa-clock-o"
                                                           aria-hidden="true"></i> <?php echo $list2[$i]['datetime'] ?></span>
                            <span class="poll_cmt_del"><?php if ($list2[$i]['del']) {
                                    echo $list2[$i]['del'] . "<i class=\"fa fa-trash-o\" aria-hidden=\"true\"></i><span class=\"sound_only\">삭제</span></a>";
                                } ?></span>
                        </header>
                        <p>
                    ...
                    <form name="fpollresult" action="./poll_etc_update.php" onsubmit="return fpollresult_submit(this);"
                        ...
                        <div id="poll_result_wcmt">
                            ...
                            <div>
                                ...
                                <input type="text" id="pc_idea" name="pc_idea" required class="full_input required"
                        ...
                        <?php if ($is_guest) { ?>
                            <div class="poll_guest">
                                ...
                                <input type="text" name="pc_name" id="pc_name" required class="full_input required"
                                       size="20" placeholder="이름">
                            </div>
        ...
        <aside id="poll_result_oth">
            ...
            <ul>
                ...
                $counter = count($list3);<?php for ($i = 0; $i < $counter; $i++) { ?>
                    <li>
                        <a href="./poll_result.php?po_id=<?php echo $list3[$i]['po_id'] ?>&amp;skin_dir=<?php echo urlencode($skin_dir); ?>"> <?php echo $list3[$i]['subject'] ?> </a><span><i
                                    class="fa fa-clock-o"
                                    aria-hidden="true"></i> <?php echo $list3[$i]['date'] ?></span></li>
                <?php } ?>
            </ul>
        </aside>
        <!-- } 설문조사 다른 결과 보기 끝 -->
